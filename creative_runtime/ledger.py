    ...
    def replay(self, *, allow_migration_bridge: bool = False) -> StoryState:
        ...
        for event in self.events[1:]:
            if event.event_type == "player_action":
                ...
                # A chain hash proves integrity, not authority.  Session-level
                # validation may approve one source-bound migration bridge, but
                # a caller-authored patch is never replay authority.
                ...
                if not isinstance(patch, Mapping):
                    raise LedgerViolation("migration_bridge requires a patch mapping")
                state = apply_state_patch(state, patch)
            else:
