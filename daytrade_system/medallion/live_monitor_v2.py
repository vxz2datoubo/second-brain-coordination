...
# ========== Web服务 ==========
def create_app(engine: SignalEngine):
    try:
    ...
    # 读取HTML
    html_path = r"F:\aidanao\daytrade_system\medallion\templates\dashboard_v2.html"
    if os.path.exists(html_path):
        with open(html_path, "r", encoding="utf-8") as f:
            HTML = f.read()
    else:
        HTML = "<html><body><h1>请创建 dashboard_v2.html</h1></body></html>"
    ...
    def get_signals():
        result = {}
        sentiment = request.args.get("sentiment", "normal")  # bullish/normal/bearish
        ...
        for code in engine.CODES:
            ...
            if today_min5:
                ind = engine.calc_indicators(today_min5, daily)
                sig = engine.generate_signal(code, ind, sentiment if sentiment != "normal" else None)
            else:
                ind = {}
                sentiment = engine.sentiment_monitor.get_sentiment() if hasattr(engine, "sentiment_monitor") else "normal"
            sig = engine.generate_signal(code, ind, sentiment)
            
            result[code] = sig
        
        sentiment_info = engine.sentiment_monitor.get_sentiment_info() if hasattr(engine, "sentiment_monitor") else {"sentiment": "normal", "label": "正常", "description": ""}
        return jsonify({
            "signals": result,
            "portfolio": engine.get_status(),
    ...
    def execute():
        ...
        if action == "sell":
            ok = engine.execute_sell(code, price, time_str)
            return jsonify({"ok": ok})
        elif action == "buyback":
            profit = engine.execute_buyback(code, price, time_str)
    ...
    def reset():
        for code in engine.CODES:
            ...
            engine.daily_stats[code] = {"sells": 0, "buys": 0, "pnl": 0.0, "trades": []}
...
def main():
    ...
    for code in engine.CODES:
        ...
        print(f"\n{'='*50}")
        print(f" {engine.CODES[code]['name']}({code})")
        print(f"{'='*50}")
        
        if today_min5:
            ind = engine.calc_indicators(today_min5, daily)
            sig = engine.generate_signal(code, ind)
            print(f"  当前价格: {sig['price']}")
            print(f"  时间: {sig['time']}")
            print(f"  开盘涨幅: {ind.get('open_to_now', 0):+.2f}%")
            print(f"  日内高点: {ind.get('day_high', 0):.3f} 回撤:{ind.get('pullback', 0):.2f}%")
            print(f"  MA5: {ind.get('ma5', 0):.3f} 偏离:{ind.get('ma5_dev', 0):+.2f}%")
            print(f"  VWAP: {ind.get('vwap', 0):.3f} 偏离:{ind.get('vwap_dev', 0):+.2f}%")
            print(f"  RSI: {ind.get('rsi', 0):.0f}")
            print(f"  量比: {ind.get('vol_ratio', 0):.1f}x")
            print(f"  卖出信号分: {sig['scores']['sell']}")
            print(f"  接回信号分: {sig['scores']['buy']}")
            print(f"  置信度: {sig['confidence']}")
            print(f"  建议: {sig['recommendation']}")
            if sig['reason']:
                print(f"  理由: {' | '.join(sig['reason'][:3])}")
            if sig['sell_target']:
                print(f"  卖出价: {sig['sell_target']} | 接回目标: {sig['buy_target']} | 止损: {sig['stop_loss']}")
        else:
            print("  无今日数据")
    ...
    if app:
        ...
        print("\n[提示] 安装 flask 和 flask-cors 以启动Web界面:")
