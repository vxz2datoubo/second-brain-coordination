...
class CreativeWebPlayerTests(unittest.TestCase):
    def test_static_player_only_accepts_verified_synthetic_catalogues(self) -> None:
        ...
        self.assertIn("Use a declared safe intent", source)
        self.assertIn("safe_intent_examples", source)
        self.assertIn("submitSafeIntent", source)
        self.assertIn("Clarification required", source)
        self.assertIn("isUnsafeIntent", source)
        ...
        self.assertIn("Verified scenarios", source)
        self.assertIn("CreativeRuntimeExperienceLibrary/v1", source)
        self.assertIn("loadLibrary", source)
        self.assertIn("experience_library.json", source)
        self.assertIn("client_story_authority === false", source)
        self.assertIn("Library contains a duplicate scenario.", source)
