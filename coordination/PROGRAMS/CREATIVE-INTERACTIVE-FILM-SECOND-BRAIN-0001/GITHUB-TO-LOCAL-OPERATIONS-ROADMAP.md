...
| Runtime tests | Python 3.13 runs `test_creative*.py`, then uploads exact-head lifecycle and synthetic-operations JSON receipts for seven days | deterministic contracts work on the declared daily runtime in clean Linux runners and leave downloadable executor evidence | real customer load, real media quality, or compatibility with a runtime not explicitly supported |
...
| Minimal session handoff | `creativectl session-receipt` emits only source-verified hashes, graph revision, event count, slot and final synthetic state | another clean environment can compare a session identity without receiving a restorable save or event text | customer-safe export, customer-session restoration, or authorization for local operations |
...
| Branch-review evidence | The same package carries `CreativeSyntheticReplayReviewBoard/v1`, with every genuine choice point and exact terminal state deltas for each legal option | a reviewer can inspect alternative endings without trusting prose or a local session; any changed delta fails exact reconstruction | the review board chooses a preferred ending, scores artistic quality, or grants product approval |
...
This fixed rehearsal creates eight independent synthetic slots alternating
between `night_signal` and `harbor_protocol`, applies a frame-versioned choice,
repeats its opaque command ID to prove idempotency, applies a second verified
choice, and finally requires a clean operations report. Its numbers (`8`
sessions, `3` events per session, `8` deduplicated retries) are deliberately
fixed evidence, not a production throughput claim.
