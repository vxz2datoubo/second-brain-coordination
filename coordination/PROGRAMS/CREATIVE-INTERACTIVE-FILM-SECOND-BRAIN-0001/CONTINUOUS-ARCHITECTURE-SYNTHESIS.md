...
| Director review board | `creativectl director-review --scenario <name>` | each card must exactly rebuild from a complete director-coverage prefix and retain its timeline hash, scene asset, shots and zero-hard-finding gate | a human can inspect camera, lighting, sound, axis, performance task and provenance across every reachable state without invoking a generator |
