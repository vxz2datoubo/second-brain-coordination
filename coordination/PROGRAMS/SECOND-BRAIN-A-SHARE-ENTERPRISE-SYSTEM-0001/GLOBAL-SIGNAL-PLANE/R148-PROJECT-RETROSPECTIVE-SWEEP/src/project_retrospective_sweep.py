...
COVERAGE_STATUSES = frozenset({
    "COMPLETE_ENUMERATION_PROVEN", "PARTIAL_ENUMERATION",
    "CALLER_SUPPLIED_EXPORT_COMPLETE", "CALLER_SUPPLIED_EXPORT_PARTIAL",
    "ENUMERATION_UNAVAILABLE",
})
COMPLETE_COVERAGE_STATUSES = frozenset({"COMPLETE_ENUMERATION_PROVEN", "CALLER_SUPPLIED_EXPORT_COMPLETE"})
...
_RECONCILIATION_ISSUER_SEAL = object()
...
class _VerifiedR142Reconciliation(dict):
    """Process-local capability proving this result passed the R148->R142 seam.

    A plain mapping with R142-looking labels is deliberately not sufficient for
    admission. This mirrors the private-token composition style already used by
    the canonical durable-admission boundary.
    """

    def __init__(self, value: Mapping[str, Any], *, seal: object, admission_capable: bool) -> None:
        if seal is not _RECONCILIATION_ISSUER_SEAL:
            raise ProjectRetrospectiveSweepError("R148_RECONCILIATION_ISSUER_SEAL_REQUIRED")
        super().__init__(json.loads(json.dumps(dict(value))))
        self._admission_capable = bool(admission_capable)
        self._admission_claimed = False

    def claim_admission_once(self) -> None:
        if not self._admission_capable:
            raise ProjectRetrospectiveSweepError("R148_CURRENT_R142_RECONCILIATION_REQUIRED", "/reconciliation")
        if self._admission_claimed:
            raise ProjectRetrospectiveSweepError("R148_RECONCILIATION_ALREADY_USED", "/reconciliation")
        self._admission_claimed = True
...
def _coverage_manifest(source: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for window in source["windows"]:
        windows.append({
            ...
            "item_refs": sorted(
                item.get("source_message_ref", "UNKNOWN") for item in window["items"] if isinstance(item, Mapping)
            ),
        })
    return {
        "project_ref": source["project_ref"], "snapshot_ref": source["snapshot_ref"],
...
def _verify_complete_coverage(
    ...
    if verifier is None:
        raise ProjectRetrospectiveSweepError("R148_TRUSTED_COMPLETE_COVERAGE_VERIFIER_REQUIRED", "/source/coverage_status")
    request = {
        "schema_version": "ProjectCoverageVerificationRequest/v1",
        "project_ref": source["project_ref"], "snapshot_ref": source["snapshot_ref"],
        "coverage_status": source["coverage_status"],
        "window_manifest_digest": _digest(_coverage_manifest(source)),
...
def validate_source_snapshot(
    ...
    *,
    ...
    required = {
        ...
        "coverage_evidence_refs", "omitted_or_unavailable_refs", "source_provider_version", "windows",
    }
...
def prepare_project_sweep(
    source_snapshot: Mapping[str, Any], *, generated_at: str, expected_canonical_main: str,
    complete_coverage_verifier: Callable[[Mapping[str, Any]], Mapping[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    for wi, window in enumerate(source["windows"]):
        for ii, item in enumerate(window["items"]):
            ...
            record = {
                "window_ref": window["window_ref"], "source_message_ref": message_ref,
                "source_time_range": time_range, "source_evidence_refs": evidence_refs,
                "original_intent_ref": original_intent_ref, "semantic_identity": identity,
                "semantic_identity_ref": identity_ref, "candidate_id": candidate_id,
                "candidate": payload, "relations": item.get("relations", []),
            }
            ...
            if directive != "CAPTURE_ALLOWED":
                ...
                excluded.append({"semantic_identity_ref": identity_ref, "source_message_ref": message_ref, "reason": directive})
    ...
    for identity_ref in sorted(groups):
        ...
        core = (payload["signal_kind"], payload["proposed_primary_domain"], payload["epistemic_state"], payload["privacy_scope"])
        for record in records[1:]:
            other = record["candidate"]
            if (other["signal_kind"], other["proposed_primary_domain"], other["epistemic_state"], other["privacy_scope"]) != core:
                raise ProjectRetrospectiveSweepError("R148_SEMANTIC_GROUP_CORE_DRIFT", f"/semantic_groups/{identity_ref}")
        evidence_refs = sorted({ref for record in records for ref in record["source_evidence_refs"]} | {
            f"source-message://sha256/{_digest(record['source_message_ref'])}" for record in records
        })
        counterevidence = sorted({ref for record in records for ref in record["candidate"]["counterevidence_refs"]})
        ...
        candidates.append({
            "candidate_id": group["candidate_id"], "source_window_ref": primary["window_ref"],
            "source_message_ref": primary["source_message_ref"], "source_project": source["source_project"],
            "source_time_range": primary["source_time_range"], "public_safe_summary": payload["public_safe_summary"],
            "original_intent_ref": primary["original_intent_ref"], "signal_kind": payload["signal_kind"],
            "epistemic_state": payload["epistemic_state"], "desired_effect": payload["desired_effect"],
            "problem_to_solve": payload["problem_to_solve"], "success_condition": payload["success_condition"],
            "expected_problems": payload["expected_problems"], "risks": payload["risks"],
            "assumptions": payload["assumptions"], "unknowns": payload["unknowns"],
            "dependencies": payload["dependencies"], "evidence_refs": evidence_refs,
            "counterevidence_refs": counterevidence, "proposed_primary_domain": payload["proposed_primary_domain"],
            "related_domains": payload["related_domains"], "privacy_scope": payload["privacy_scope"],
            "historical_status": payload["historical_status"], "candidate_relations": sorted(relations, key=_canonical),
    ...
    if candidates:
        package = {
            "schema_version": "SignalImportPackage/v1", "import_batch_id": sweep_id,
            ...
            "package_metadata": {
                "r148_project_ref": source["project_ref"], "r148_snapshot_ref": source["snapshot_ref"],
    ...
    return {
        "schema_version": PLAN_SCHEMA, "sweep_id": sweep_id,
        "project_ref": source["project_ref"], "snapshot_ref": source["snapshot_ref"],
        "coverage_status": source["coverage_status"], "project_scan_complete": source["project_scan_complete"],
        "coverage_attestation": source["coverage_attestation"], "window_count_observed": source["window_count_observed"],
        "source_snapshot_digest": source["source_snapshot_digest"], "candidate_count": len(candidates),
        "candidate_ids": [item["candidate_id"] for item in candidates],
        "excluded_items": sorted(excluded, key=lambda item: (item["semantic_identity_ref"], item["source_message_ref"])),
        "package": package, "expected_canonical_main": expected_canonical_main,
        "no_capture_work": not candidates,
        "no_second_signal_truth_created": True, "r142_disposition_authority_preserved": True,
    }


def _reconciliation_binding(value: Mapping[str, Any]) -> str:
    return _digest({
        "sweep_id": value["sweep_id"], "expected_canonical_main": value["expected_canonical_main"],
        "package_digest": value["package_digest"], "snapshot_digest": value["snapshot_digest"],
        "canonical_snapshot_or_main": value["canonical_snapshot_or_main"],
        "results": value["results"],
    })


def reconcile_project_sweep(
    plan: Mapping[str, Any], current_snapshot: Mapping[str, Any], *, expected_canonical_main: str,
    live_observation_proof: Any = None, exact_read_proofs: Sequence[Any] = (), ledger: Any = None,
    ...
    if plan.get("expected_canonical_main") != expected_canonical_main:
        raise ProjectRetrospectiveSweepError("R148_PLAN_CANONICAL_BINDING_MISMATCH", "/expected_canonical_main")
    if plan.get("candidate_count") == 0:
        if plan.get("package") is not None or plan.get("candidate_ids"):
            raise ProjectRetrospectiveSweepError("R148_ZERO_CANDIDATE_PLAN_INVALID", "/package")
        data = {
            "schema_version": RECONCILIATION_SCHEMA, "sweep_id": plan["sweep_id"],
            "expected_canonical_main": expected_canonical_main,
            "canonical_snapshot_or_main": expected_canonical_main,
            "snapshot_digest": "NO_CANDIDATES", "package_digest": "NO_CANDIDATES",
            "disposition_counts": {}, "results": [], "no_capture_work": True,
        }
        data["reconciliation_binding_digest"] = _reconciliation_binding(data)
        return _VerifiedR142Reconciliation(data, seal=_RECONCILIATION_ISSUER_SEAL, admission_capable=True)
    package = plan.get("package")
    if not isinstance(package, Mapping) or package.get("expected_canonical_main") != expected_canonical_main:
        raise ProjectRetrospectiveSweepError("R148_PLAN_CANONICAL_BINDING_MISMATCH", "/package/expected_canonical_main")
    parsed = validate_import_package(package)
    result = reconcile_fn(
        package, current_snapshot, expected_canonical_main=expected_canonical_main,
    ...
    if result.get("package_digest") != parsed["package_digest"]:
        raise ProjectRetrospectiveSweepError("R148_R142_PACKAGE_DIGEST_MISMATCH", "/reconciliation/package_digest")
    if result.get("canonical_snapshot_or_main") != expected_canonical_main:
        raise ProjectRetrospectiveSweepError("R148_R142_CANONICAL_BINDING_MISMATCH", "/reconciliation/canonical_snapshot_or_main")
    expected = {item["candidate_id"]: item["candidate_digest"] for item in parsed["candidates"]}
    seen: set[str] = set(); counts: dict[str, int] = {}; normalized_results = []
    for index, decision in enumerate(result["results"]):
        ...
        seen.add(candidate_id); counts[disposition] = counts.get(disposition, 0) + 1
    ...
    data = {
        "schema_version": RECONCILIATION_SCHEMA, "sweep_id": plan["sweep_id"],
        "expected_canonical_main": expected_canonical_main,
        "canonical_snapshot_or_main": result["canonical_snapshot_or_main"],
        "snapshot_digest": result.get("snapshot_digest", "UNKNOWN"),
        "package_digest": result["package_digest"],
        "disposition_counts": dict(sorted(counts.items())), "results": normalized_results,
        "no_capture_work": False,
    }
    data["reconciliation_binding_digest"] = _reconciliation_binding(data)
    return _VerifiedR142Reconciliation(
        data, seal=_RECONCILIATION_ISSUER_SEAL,
        admission_capable=(reconcile_fn is reconcile_package),
    )


def _require_verified_reconciliation(plan: Mapping[str, Any], reconciliation: Mapping[str, Any]) -> _VerifiedR142Reconciliation:
    if type(reconciliation) is not _VerifiedR142Reconciliation:
        raise ProjectRetrospectiveSweepError("R148_VERIFIED_R142_RECONCILIATION_REQUIRED", "/reconciliation")
    if reconciliation.get("schema_version") != RECONCILIATION_SCHEMA or reconciliation.get("sweep_id") != plan.get("sweep_id"):
        raise ProjectRetrospectiveSweepError("R148_RECONCILIATION_SWEEP_MISMATCH", "/reconciliation")
    if reconciliation.get("expected_canonical_main") != plan.get("expected_canonical_main"):
        raise ProjectRetrospectiveSweepError("R148_R142_CANONICAL_BINDING_MISMATCH", "/reconciliation/expected_canonical_main")
    if reconciliation.get("reconciliation_binding_digest") != _reconciliation_binding(reconciliation):
        raise ProjectRetrospectiveSweepError("R148_R142_RECONCILIATION_BINDING_INVALID", "/reconciliation")
    if plan.get("candidate_count") == 0:
        if reconciliation.get("results") != [] or reconciliation.get("package_digest") != "NO_CANDIDATES":
            raise ProjectRetrospectiveSweepError("R148_ZERO_CANDIDATE_RECONCILIATION_INVALID", "/reconciliation")
        return reconciliation
    parsed = validate_import_package(plan["package"])
    if reconciliation.get("package_digest") != parsed["package_digest"]:
        raise ProjectRetrospectiveSweepError("R148_R142_PACKAGE_DIGEST_MISMATCH", "/reconciliation/package_digest")
    expected = {item["candidate_id"]: item["candidate_digest"] for item in parsed["candidates"]}
    seen = set()
    for index, decision in enumerate(reconciliation.get("results", [])):
        candidate_id = decision.get("candidate_id")
        if candidate_id not in expected or candidate_id in seen or decision.get("candidate_digest") != expected[candidate_id]:
            raise ProjectRetrospectiveSweepError("R148_R142_RESULT_BINDING_INVALID", f"/reconciliation/results/{index}")
...
def build_r147_requests(plan: Mapping[str, Any], reconciliation: Mapping[str, Any]) -> list[dict[str, Any]]:
    if plan.get("schema_version") != PLAN_SCHEMA:
        raise ProjectRetrospectiveSweepError("R148_PLAN_SCHEMA_INVALID", "/schema_version")
    verified = _require_verified_reconciliation(plan, reconciliation)
    if plan.get("candidate_count") == 0:
        return []
    parsed = validate_import_package(plan["package"])
    candidates = {item["candidate_id"]: item for item in parsed["candidates"]}
    new_rows = [row for row in verified["results"] if row.get("disposition") == "NEW_DURABLE_SIGNAL"]
    if new_rows and not verified._admission_capable:
        raise ProjectRetrospectiveSweepError("R148_CURRENT_R142_RECONCILIATION_REQUIRED", "/reconciliation")
    requests = []
    for decision in new_rows:
        candidate_id = decision["candidate_id"]; candidate = candidates[candidate_id]
        ...
        if candidate.get("candidate_relations"):
            raise ProjectRetrospectiveSweepError("R148_RELATION_CAPABLE_CANONICAL_ADMISSION_REQUIRED", f"/candidates/{candidate_id}/candidate_relations")
        evidence_refs = sorted(set(candidate["evidence_refs"]) | {
            f"r142-reconciliation://sha256/{verified['reconciliation_binding_digest']}"
        })
        request = {
            "schema_version": REQUEST_SCHEMA, "attempt_id": f"{plan['sweep_id']}-{candidate_id[-12:]}",
            "capture_identity": f"r148:{candidate_id}", "capture_command": "把这个录入信号塔",
            "source_project": candidate["source_project"], "source_window_ref": candidate["source_window_ref"],
            ...
            "dependencies": candidate["dependencies"], "evidence_refs": evidence_refs,
            ...
            "epistemic_state": candidate["epistemic_state"], "signal_kind": candidate["signal_kind"],
        }
...
def _validate_r147_receipt(request: Mapping[str, Any], receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    refs = receipt.get("authority_refs")
    ...
    if receipt.get("readback_verification_status") != "VERIFIED_SAME_LEDGER" or receipt.get("fresh_replay_verification_status") != "VERIFIED_FRESH_S0C_REPLAY":
        raise ProjectRetrospectiveSweepError("R148_R147_DURABLE_VERIFICATION_FAILED", "/admission_receipts/readback")
    if not all(receipt.get(field) is False for field in ("task_created", "route_created", "work_claim_created", "write_permission_created")):
    ...
    if receipt["status"] == "ADMITTED":
        if after["event_count"] != before["event_count"] + 1 or offset != after["event_count"]:
            raise ProjectRetrospectiveSweepError("R148_R147_REPLAY_BINDING_MISMATCH", "/admission_receipts/transport_replay_after")
    elif after["event_count"] != before["event_count"]:
        raise ProjectRetrospectiveSweepError("R148_R147_REPLAY_BINDING_MISMATCH", "/admission_receipts/transport_replay_after")
    return dict(receipt)


def finalize_sweep_receipt(
    plan: Mapping[str, Any], reconciliation: Mapping[str, Any], admission_receipts: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    verified = _require_verified_reconciliation(plan, reconciliation)
    expected_requests = build_r147_requests(plan, verified)
    requests_by_attempt = {item["attempt_id"]: item for item in expected_requests}
    receipts_by_attempt: dict[str, dict[str, Any]] = {}; failures: list[dict[str, str]] = []
    ...
    admitted_refs = []; replay_checksums = []
    for attempt_id, receipt in sorted(receipts_by_attempt.items()):
        request = requests_by_attempt[attempt_id]
        admitted_refs.append({
            "capture_identity": request["capture_identity"], "attempt_id": attempt_id,
            "status": receipt["status"], "signal_id": receipt["signal_id"],
            "event_id": receipt["event_id"], "receipt_id": receipt["receipt_id"],
            "receipt_offset": receipt["receipt_offset"],
        })
        replay_checksums.append(receipt["transport_replay_after"]["projection_checksum"])
    counts = verified.get("disposition_counts", {})
    new_count = int(counts.get("NEW_DURABLE_SIGNAL", 0)) if isinstance(counts, Mapping) else 0
    return {
        "schema_version": RECEIPT_SCHEMA, "project_ref": plan["project_ref"], "sweep_id": plan["sweep_id"],
        "snapshot_ref": plan["snapshot_ref"], "coverage_status": plan["coverage_status"],
        "project_scan_complete": bool(plan["project_scan_complete"]), "coverage_attestation": plan.get("coverage_attestation"),
        "window_count_observed": plan["window_count_observed"], "candidate_count": plan["candidate_count"],
        "disposition_counts": counts, "new_durable_signal_count": new_count,
        "new_durable_signal_ids": sorted(row["candidate_id"] for row in verified.get("results", []) if row.get("disposition") == "NEW_DURABLE_SIGNAL"),
        "admitted_signal_refs": admitted_refs, "admission_failures": failures,
        "missing_receipt_attempt_ids": missing_attempts,
        "durable_admission_complete_for_observed_new": not failures and not missing_attempts and len(admitted_refs) == new_count,
        "replay_projection_checksums": sorted(set(replay_checksums)),
        "automatic_task_created": False, "automatic_route_created": False,
        "automatic_work_claim_created": False, "automatic_write_permission_created": False,
        "second_signal_truth_created": False,
        "coverage_caveat": "NONE" if plan["project_scan_complete"] else "PARTIAL_OR_UNAVAILABLE_PROJECT_ENUMERATION; THIS RECEIPT DOES NOT CLAIM COMPLETE PROJECT COVERAGE",
    }


def execute_r147_admissions(
    plan: Mapping[str, Any], reconciliation: Mapping[str, Any], *,
    admit_fn: Callable[[Mapping[str, Any]], Mapping[str, Any]],
) -> dict[str, Any]:
    """Execute only a sealed current-R142, relation-safe NEW batch through R147."""
    verified = _require_verified_reconciliation(plan, reconciliation)
    requests = build_r147_requests(plan, verified)
    if requests:
        verified.claim_admission_once()
    wrapped: list[dict[str, Any]] = []
    for request in requests:
        receipt = admit_fn(request)
        if not isinstance(receipt, Mapping):
            raise ProjectRetrospectiveSweepError("R148_R147_RECEIPT_INVALID", "/admission_receipts")
        bound = dict(receipt); bound["r148_capture_identity"] = request["capture_identity"]
        wrapped.append(bound)
    return finalize_sweep_receipt(plan, verified, wrapped)
