...
class R147MalformedBatchIsolationTests(unittest.TestCase):
    def test_non_utf8_request_is_isolated_and_valid_peers_persist(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            git(seed, "commit", "-q", "-m", "valid request before non-utf8")

            bad_attempt = "nonutf8-bad"
            bad_relative = f"{R147_ROOT}/transport/requests/{bad_attempt}.json"
            bad_path = seed / bad_relative
            bad_path.parent.mkdir(parents=True, exist_ok=True)
            bad_path.write_bytes(
                b'{"attempt_id":"nonutf8-bad","public_safe_summary":"' + bytes([0xFF]) + b'"}\n'
            )
            git(seed, "add", bad_relative)
            git(seed, "commit", "-q", "-m", "invalid utf8 request")

            last_attempt = "nonutf8-good-c"
            ...
            git(seed, "commit", "-q", "-m", "valid request after non-utf8")
            ...
            self.assertNotIn("xff", json.dumps(bad, sort_keys=True).casefold())
            ...
            self.assertEqual(2, len(rows), "invalid UTF-8 request must not enter S0C replay transport")
