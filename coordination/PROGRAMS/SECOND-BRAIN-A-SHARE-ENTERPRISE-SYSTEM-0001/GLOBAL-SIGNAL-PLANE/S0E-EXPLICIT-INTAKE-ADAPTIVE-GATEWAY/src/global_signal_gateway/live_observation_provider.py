...
_REQUIRED_AUTHORITY_BINDING_FIELDS = frozenset({
    "domain_id",
    "project_id",
    "repository",
    "canonical_commit",
...
def _safe_contract_path(path: Any) -> str:
    ...
    if (
        ...
        or not path.startswith("coordination/TASK-BRIEFS/")
...
def _domain_authority_spec(
    contract: Mapping[str, Any],
    *,
    domain_id: str,
    repository: str,
) -> tuple[str, bool]:
    """Resolve one authority path only from a canonical task brief."""
...
class LiveObservationRequest:
    ...
    def validate(self) -> None:
        if (
            not self.request_id
            or self.provider_contract_revision != CONTRACT_REVISION
            or self.target_repository != TARGET_REPOSITORY
            or self.target_branch != "main"
        ):
        ...
        for target in self.required_domain_freshness_targets:
            ...
            if not governed:
                ...
                continue
            if (
                not isinstance(target.domain_id, str)
                or not target.domain_id.strip()
                or target.authority_contract_path is None
            ):
...
class ExactObjectRecord:
    ...
    def ref(self) -> str:
        return (
            f"github://{self.repository}@{self.commit_sha}/{self.path}"
            f"#blob={self.blob_sha};sha256={self.content_sha256}"
        )
...
class LiveObservationEvidenceBundle:
    ...
    def exact_refs(self) -> tuple[str, ...]:
        return (
            (self.identity_ref(),)
            + tuple(record.ref() for record in self.exact_objects)
            + tuple(self.domain_authority_refs)
        )
...
class LiveObservationProvider:
    ...
    def _dynamic_domain_endpoint_allowed(self, path: str) -> bool:
        ...
        for repository in repositories:
            ...
            if path == f"{prefix}/git/ref/heads/main":
                return True
            if re.fullmatch(re.escape(prefix) + r"/git/commits/[0-9a-f]{40,64}", path):
                return True
            if re.fullmatch(re.escape(prefix) + r"/git/trees/[0-9a-f]{40,64}\?recursive=1", path):
                return True
            if re.fullmatch(re.escape(prefix) + r"/git/blobs/[0-9a-f]{40,64}", path):
                return True
        return False
    ...
    def _ref(self, repository: str, branch: str) -> tuple[str, Mapping[str, Any]]:
        ...
        return (
            _require_sha(
                _mapping(obj.get("object"), "GITHUB_BRANCH_RESPONSE_INVALID").get("sha"),
    ...
    def _commit_tree(self, repository: str, commit: str) -> tuple[str, Mapping[str, Any]]:
        ...
        return (
            _require_sha(
                _mapping(obj.get("tree"), "GITHUB_COMMIT_RESPONSE_INVALID").get("sha"),
    ...
    def _tree(self, repository: str, tree_sha: str) -> tuple[Mapping[str, str], Mapping[str, Any]]:
        _, value, metadata = self._get_json(
    ...
    ) -> tuple[tuple[RawReviewRecord, ...], tuple[Mapping[str, Any], ...]]:
        page, records, evidence = 1, [], []
        while page <= MAX_PAGES:
            headers, value, metadata = self._get_json(
            ...
            for item in value:
                ...
                records.append(
                    RawReviewRecord(
                        review_id,
                        state,
                        item.get("commit_id") if isinstance(item.get("commit_id"), str) else None,
    ...
    def _pr(self, repository: str, number: int) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
        ...
        head = _require_sha(
            _mapping(pr.get("head"), "GITHUB_PR_RESPONSE_INVALID").get("sha"),
            "GITHUB_PR_RESPONSE_INVALID",
        )
        base = _require_sha(
            _mapping(pr.get("base"), "GITHUB_PR_RESPONSE_INVALID").get("sha"),
            "GITHUB_PR_RESPONSE_INVALID",
        )
        state = pr.get("state")
    ...
    def _coordinator_yaml_object(
        ...
        *,
        ...
        if not any(
            prior.repository == record.repository
            and prior.commit_sha == record.commit_sha
            and prior.path == record.path
            and prior.blob_sha == record.blob_sha
    ...
    def observe(
        self, request: LiveObservationRequest
    ) -> tuple[LiveObservationEvidenceBundle, AuthorityBoundLiveObservationProof]:
        request.validate()
        started = _utc_now()
        metadata: list[Mapping[str, Any]] = []
        ...
        if (
            active.get("task_id") != request.expected_task_id
            or active.get("route_epoch") != request.expected_route_epoch
        ):
        ...
        metadata.append(item)
        reviews, review_metadata = self._reviews(
            request.target_repository, request.pull_request_number
        )
        ...
        for target in request.required_domain_freshness_targets:
            ...
            if target.authority_contract_path is not None:
                ...
                contract = self._coordinator_yaml_object(
                    path=contract_path,
                    commit=initial_main,
                    tree_sha=main_tree,
                    tree=tree,
                ...
                authority_path, private = _domain_authority_spec(
                    contract,
            ...
            metadata.append(item)
            domain_refs.append(
                f"github://{target.repository}@{domain_sha}:refs/heads/{target.branch}"
            )
            ...
            if authority_path is not None:
                ...
                authority_blob = _require_sha(
                    domain_tree[authority_path], "GITHUB_TREE_RESPONSE_INVALID"
                )
                authority_payload, item = self._blob(target.repository, authority_blob)
                metadata.append(item)
                authority_record = ExactObjectRecord(
                    target.repository,
                    domain_sha,
                    domain_tree_sha,
                    authority_path,
                ...
                native = native_semantic_authority_identity(
                    authority_payload, path=authority_path
                )
                assert target.domain_id is not None
                governed_identity = {
                    "domain_id": target.domain_id,
                    "project_id": native["project_id"],
                    "authority_schema_version": native["authority_schema_version"],
                    "writeback_owner": target.domain_id,
                ...
                domain_authority_refs.append(
                    governed_semantic_authority_ref(
                        authority_record.ref(), governed_identity
                    )
                )
        ...
        lease_fp = digest(
            {
                ...
                "control_tower": next(
                    ...
                    if record.path == CONTROL_PATHS[3]
                ),
            }
        )
        pending = digest(
            {
                "active": active.get("pending_approvals", []),
                "route": route.get("pending_approvals", []),
            }
        )
        invalidators = {
            "pr_number": pr_first["number"],
            "pr_state": pr_first["state"],
            "head_sha": pr_first["head_sha"],
            "base_sha": pr_first["base_sha"],
            "current_main_sha": initial_main,
            "review_state_ref": digest([record.__dict__ for record in reviews]),
            "merged": pr_first["merged"],
            "merge_commit_sha": pr_first["merge_commit_sha"],
            "route_fingerprint": route_fp,
            "claim_fingerprint": claim_fp,
            "lane_fingerprint": lane_fp,
            "lease_fingerprint": lease_fp,
            "domain_freshness_ref": digest(domain_refs),
            "pending_approval_ref": pending,
        }
        observation_basis = {
        ...
        payload = {
            ...
            "code_ref": "global_signal_gateway/live_observation_provider.py",
            ...
            "domain_authority_refs": list(domain_authority_refs),
            "pr": pr_first,
            "reviews": [record.__dict__ for record in reviews],
            "route": route_fp,
            "claim": claim_fp,
            "lane": lane_fp,
            "lease": lease_fp,
            "domain": digest(domain_refs),
            "pending": pending,
        ...
        bundle = LiveObservationEvidenceBundle(
            PROVIDER_ID,
            CONTRACT_REVISION,
            ...
            _iso(fresh_until),
            invalidators,
            tuple(domain_authority_refs),
            digest(payload),
        )
        _BUNDLES[bundle.identity_ref()] = bundle
        proof = AuthorityBoundLiveObservationProof(
            bundle.target_repository,
            request.pull_request_number,
            str(pr_first["state"]),
            str(pr_first["head_sha"]),
            str(pr_first["base_sha"]),
            initial_main,
            bool(pr_first["merged"]),
            pr_first["merge_commit_sha"],
            invalidators["review_state_ref"],
            bundle.completed_at,
            route_fp,
            claim_fp,
            lane_fp,
            lease_fp,
            bundle.domain_freshness_ref,
            pending,
            bundle.exact_refs(),
            PROVIDER_ID,
            bundle.identity_ref(),
...
def verify_r137_proof(
    proof: AuthorityBoundLiveObservationProof, checked_at: datetime
) -> bool:
    """Static production verifier. No registration API and no network call."""
    if (
        proof.provider_id != PROVIDER_ID
        or not proof.provider_attribution_ref.startswith("provider://r137/evidence/")
    ):
        return False
    ...
    try:
        ...
        return False
    return bool(
        bundle.provider_contract_revision == CONTRACT_REVISION
        and bundle.provider_code_digest == _provider_code_digest()
        and bundle.initial_main_sha == bundle.final_main_sha
        and completed <= checked_at <= fresh_until
        and bundle.pagination_complete
        and not bundle.warnings
        and tuple(proof.exact_refs) == bundle.exact_refs()
        and proof.invalidation_fingerprints == bundle.invalidation_fingerprints
        and proof.repository == bundle.target_repository
        and proof.current_main_sha == bundle.initial_main_sha
        and proof.pr_number == bundle.pr["number"]
        and proof.pr_state == bundle.pr["state"]
        and proof.head_sha == bundle.pr["head_sha"]
        and proof.base_sha == bundle.pr["base_sha"]
        and proof.merged == bundle.pr["merged"]
        and proof.merge_commit_sha == bundle.pr["merge_commit_sha"]
        and proof.review_state_ref
        == bundle.invalidation_fingerprints["review_state_ref"]
        and proof.route_fingerprint == bundle.route_fingerprint
        and proof.claim_fingerprint == bundle.claim_fingerprint
        and proof.lane_fingerprint == bundle.lane_fingerprint
        and proof.lease_fingerprint == bundle.lease_fingerprint
        and proof.domain_freshness_ref == bundle.domain_freshness_ref
        and proof.pending_approval_ref == bundle.pending_approval_ref
    )
