...
def resolve(order):
    if not isinstance(order.match_mode, MatchMode): return OutcomeStatus.INVALID_OR_BLOCKED, 0, order.quantity, "UNSUPPORTED_MATCH_MODE"
    if order.match_mode is MatchMode.UNKNOWN: return OutcomeStatus.INVALID_OR_BLOCKED, 0, order.quantity, "UNKNOWN_MATCH_MODE"
    ...
    if order.match_mode is MatchMode.PARTIAL:
        ...
        if not isinstance(filled, int) or isinstance(filled, bool) or filled <= 0 or filled >= order.quantity: return OutcomeStatus.INVALID_OR_BLOCKED, 0, order.quantity, "INVALID_PARTIAL_FILL_QUANTITY"
