...
class TerminalExecutionReconciler:
    ...
    def reconcile(
        ...
        # E46 permanently removes the process-local E43/E44 positive chain.
        # Only a receipt minted after the same durable execution lease commits
        # may reconcile a terminal observation.
        from .execution_lease import TerminalCommitReceipt
        ...
        if not isinstance(owner_terminal, TerminalCommitReceipt):
            raise AttestationError(AttestationCode.TERMINAL_MISMATCH, "E46 durable terminal commit receipt required")
        if (
            ...
            raise AttestationError(AttestationCode.TERMINAL_MISMATCH, "E46 terminal receipt mismatch")
        if raw.evidence_type is not owner_terminal.evidence_type:
            raise AttestationError(AttestationCode.TERMINAL_MISMATCH, "E46 owner evidence type mismatch")
        if raw.exit_code != owner_terminal.exit_code:
            raise AttestationError(AttestationCode.TERMINAL_MISMATCH, "E46 terminal exit semantics mismatch")
        expected = {
