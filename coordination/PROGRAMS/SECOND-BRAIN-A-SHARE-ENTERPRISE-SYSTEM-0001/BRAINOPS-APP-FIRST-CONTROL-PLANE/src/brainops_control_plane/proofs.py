...
class RouteAuthority:
    ...
    def __post_init__(self) -> None:
        ...
        if self.automatic_dispatch_allowed or self.canary_execution_allowed:
            raise ValidationError("pre-canary route must keep automatic and canary execution disabled")
...
def parse_route_authority(active_task_content: bytes, coordination_content: bytes) -> RouteAuthority:
