...
class ClaimBoundCapabilityDecision:
    ...
    def __init__(self, challenge_decision: ChallengeCapabilityDecision, claim: DurableClaimRecord, bound_at: str, *, _seal: object | None = None) -> None:
        ...
        if claim.invocation_id is None:
            raise ValidationError("claim-bound decision requires a nonempty attached invocation")
        require_identifier(claim.invocation_id, "claim-bound invocation_id")
...
def validate_owner_terminal_evidence(claim: DurableClaimRecord, capability: object, evidence: object) -> object:
    """Validate only E46 lease-bound, verifier-minted terminal evidence.

    The old `ClaimBoundCapabilityDecision` and caller-populated terminal
    dataclasses are permanently rejected.  A positive return is still only an
    observation; the lease CAS and journaled terminal mutation remain
    mandatory downstream.
    """

    from .execution_lease import AttestedTerminalEvidence, ExecutionLeaseRecord, ExecutionLeaseState

    if not isinstance(capability, ExecutionLeaseRecord) or capability.state is not ExecutionLeaseState.INVOCATION_ATTACHED:
        raise ValidationError("invocation-attached durable execution lease required")
    if not isinstance(evidence, AttestedTerminalEvidence):
        raise ValidationError("verifier-minted terminal evidence required")
    if (
        claim.state is not DurableClaimState.CLAIMED
        or claim.claim_id != capability.claim_id
        or claim.holder != capability.holder
        or claim.invocation_id != capability.invocation_id
        or claim.provenance.digest != capability.provenance_digest
        or claim.key.storage_id != capability.storage_id
        or evidence.lease_id != capability.lease_id
        or evidence.claim_id != claim.claim_id
        or evidence.invocation_id != claim.invocation_id
        or evidence.holder != claim.holder
        or evidence.target is not capability.target
        or evidence.transport_id != capability.capability_transport_id
    ):
        raise ValidationError("terminal evidence is not bound to current lease and claim")
    return evidence
