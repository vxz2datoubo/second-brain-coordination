...
class DurableClaimAuthority:
    ...
    def acquire_effect_permit(self, provenance: VerifiedAuthorityProvenance, claim_id: str, holder: ClaimHolder, issued_at: str) -> DurableEffectPermit | None:
        if not isinstance(holder, ClaimHolder):
            return None
        result = self.read(provenance)
        if result.record is None or result.record.state is not DurableClaimState.CLAIMED or not self._same_holder(result.record, claim_id, holder):
            return None
        if result.record.provenance != provenance.binding:
            return None
        issued = parse_rfc3339_utc(issued_at, "effect permit issued_at")
        if issued < parse_rfc3339_utc(result.record.claimed_at, "durable claimed_at"):
            return None
        if issued >= parse_rfc3339_utc(provenance.binding.expires_at, "approval expires_at"):
            return None
        return DurableEffectPermit(result.record, issued_at, _seal=_PERMIT_SEAL)
    ...
    def attach_invocation(self, provenance: VerifiedAuthorityProvenance, claim_id: str, holder: ClaimHolder, invocation_id: str) -> DurableClaimResult:
        ...
        if current.record is not None and self._same_holder(current.record, claim_id, holder) and current.record.invocation_id is not None:
            return DurableClaimResult(
                DurableClaimResultCode.DUPLICATE_INVOCATION if current.record.invocation_id == invocation_id else DurableClaimResultCode.INVOCATION_MISMATCH,
                current.record,
            )
        return self._mutate(provenance, claim_id, holder, lambda record: replace(record, invocation_id=invocation_id), DurableClaimResultCode.INVOCATION_ATTACHED)
    ...
    def finalize(
        ...
        *,
        ...
        if not state.terminal:
            raise ValidationError("durable finalization requires terminal state")
        require_identifier(terminal_reason, "durable terminal_reason")
        parse_rfc3339_utc(terminal_at, "durable terminal_at")

        def transform(record: DurableClaimRecord) -> DurableClaimRecord:
            attached = record.invocation_id
            if attached is not None and invocation_id != attached:
                raise _DurableMutationError(DurableClaimResultCode.INVOCATION_MISMATCH, "invocation does not match durable claim")
            if attached is None and invocation_id is not None:
                raise _DurableMutationError(DurableClaimResultCode.INVOCATION_MISMATCH, "invocation must be attached before finalization")
            return replace(record, state=state, terminal_reason=terminal_reason, terminal_at=terminal_at)
    ...
    def recover_expired_claim(
        ...
        observed = parse_rfc3339_utc(observed_at, "durable recovery observed_at")
        current = self.read(provenance)
        if current.record is None:
            return current
        if not self._same_holder(current.record, claim_id, holder):
            return DurableClaimResult(DurableClaimResultCode.CLAIM_OWNER_MISMATCH, current.record)
        if current.record.state.terminal:
            return DurableClaimResult(DurableClaimResultCode.TERMINAL_EXISTS, current.record)
        if observed < parse_rfc3339_utc(current.record.claimed_at, "durable claimed_at") + timedelta(seconds=timeout_seconds):
            return DurableClaimResult(DurableClaimResultCode.ALREADY_CLAIMED, current.record)
        return self.finalize(
            ...
            observed_at,
            invocation_id=current.record.invocation_id,
        )
    ...
    def governed_recover_expired_claim(
        ...
        The recovery authority is bound to one original claim and holder. It may
        not attach evidence, finalize an arbitrary result, or obtain an effect
        permit because it is not a `ClaimHolder`.
        """

        if not isinstance(recovery, RecoveryAuthorization):
        ...
        if observed < parse_rfc3339_utc(recovery.not_before, "recovery not_before"):
            return DurableClaimResult(DurableClaimResultCode.RECOVERY_TIMEOUT_NOT_REACHED, None)
        if observed >= parse_rfc3339_utc(recovery.expires_at, "recovery expires_at"):
            return DurableClaimResult(DurableClaimResultCode.RECOVERY_UNAUTHORIZED, None)
        key = self._key(provenance)
        object_id = self._object_id(key)
        try:
            ...
            return DurableClaimResult(DurableClaimResultCode.AUTHORITY_UNAVAILABLE, None)
        if current.claim_id != recovery.claim_id or current.holder != recovery.original_holder:
        ...
        updated = replace(
            ...
            terminal_reason=recovery.reason,
