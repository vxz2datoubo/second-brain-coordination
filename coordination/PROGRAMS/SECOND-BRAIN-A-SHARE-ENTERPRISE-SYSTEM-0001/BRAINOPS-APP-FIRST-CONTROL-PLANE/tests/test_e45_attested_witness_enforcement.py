...
from test_e44_durable_challenge import CANARY, HASH, NONCE, TASK, _claimed, _holder, _seed_terminal_record  # noqa: E402
...
class E45AttestedWitnessEnforcementTests(unittest.TestCase):
    ...
    def _bound_decision(self, root: Path, claim, challenge=None):
        if claim.invocation_id is None:
            claim = replace(claim, invocation_id="invoke.e45.one")
    ...
    def _finalized(root: Path, holder=None):
        ...
        final = _seed_terminal_record(
            authority,
            claimed.record,
            DurableClaimState.SUCCEEDED,
            "synthetic_terminal",
            "2026-08-02T12:05:00Z",
            invocation_id="invoke.e45.one",
        )
        return authority, provenance, holder, final
