...
class E43TerminalAttestationTests(unittest.TestCase):
    ...
    def _attached_claim(self, root: Path):
        ...
        attached = authority.attach_invocation(provenance, "claim.e42.one", holder, "invocation.e43.one")
        self.assertEqual(attached.code, DurableClaimResultCode.INVOCATION_ATTACHED)
        self.assertIsNotNone(attached.record)
        return authority, provenance, holder, attached.record
    ...
    def test_matching_durable_terminal_allows_positive_classification(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            authority, provenance, holder, _, observed = self._started_and_observed(Path(directory))
            ...
            capability = ledger.consume(challenge, _e44_witness(challenge), "2026-08-02T12:05:01Z").decision
            owner_terminal = validate_owner_terminal_evidence(finalized.record, capability, _manual_terminal_evidence(finalized.record))
            reconciliation = TerminalExecutionReconciler.reconcile(observed, finalized.record, "2026-08-02T12:05:02Z", owner_terminal)
            result = TerminalExecutionReconciler.classify(finalized.record, reconciliation)
        self.assertEqual(result.code, AttestationCode.DURABLE_TERMINAL_RECONCILED)
        self.assertEqual(result.evidence_type, ExecutionEvidenceType.CURRENT_CODEX_APP_SESSION_MANUAL_EXECUTION)

    def test_manual_terminal_with_process_exit_code_fails_before_positive_reconciliation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            authority, provenance, holder, _, observed = self._started_and_observed(Path(directory), exit_code=0)
            finalized = authority.finalize(
                provenance,
                "claim.e42.one",
                holder,
            ...
            owner_terminal = validate_owner_terminal_evidence(finalized.record, capability, _manual_terminal_evidence(finalized.record))
            with self.assertRaises(AttestationError) as raised:
                TerminalExecutionReconciler.reconcile(observed, finalized.record, "2026-08-02T12:05:02Z", owner_terminal)
    ...
    def test_terminal_status_mismatch_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            finalized = authority.finalize(provenance, "claim.e42.one", holder, DurableClaimState.SUCCEEDED, "observed_completed", "2026-08-02T12:05:00Z", invocation_id="invocation.e43.one")
            with self.assertRaises(AttestationError) as raised:
                TerminalExecutionReconciler.reconcile(observed, finalized.record, "2026-08-02T12:05:02Z")
        self.assertEqual(raised.exception.code, AttestationCode.TERMINAL_MISMATCH)

    def test_terminal_time_mismatch_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            authority, provenance, holder, _, observed = self._started_and_observed(Path(directory))
            finalized = authority.finalize(provenance, "claim.e42.one", holder, DurableClaimState.SUCCEEDED, "observed_completed", "2026-08-02T12:05:03Z", invocation_id="invocation.e43.one")
            with self.assertRaises(AttestationError) as raised:
                TerminalExecutionReconciler.reconcile(observed, finalized.record, "2026-08-02T12:05:04Z")
        self.assertEqual(raised.exception.code, AttestationCode.TERMINAL_MISMATCH)
    ...
    def test_late_callback_cannot_match_durable_terminal(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            finalized = authority.finalize(provenance, "claim.e42.one", holder, DurableClaimState.SUCCEEDED, "observed_completed", "2026-08-02T12:05:00Z", invocation_id="invocation.e43.one")
            with self.assertRaises(AttestationError) as raised:
                TerminalExecutionReconciler.reconcile(observed, finalized.record, "2026-08-02T12:05:07Z")
        self.assertEqual(raised.exception.code, AttestationCode.TERMINAL_MISMATCH)
    ...
    def test_governed_recovery_can_mark_expired_claim_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            authorization = _governed_recovery_authorization(principal, "claim.e42.one", holder, "2026-08-02T12:05:00Z", "2026-08-02T12:10:00Z", "holder_timeout")
            early = authority.governed_recover_expired_claim(provenance, authorization, "2026-08-02T12:04:00Z")
            recovered = authority.governed_recover_expired_claim(provenance, authorization, "2026-08-02T12:06:00Z")
    ...
    def test_recovery_cannot_impersonate_holder_or_attach_execution(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            authority, provenance, holder, _ = _claimed(Path(directory))
            principal = _governed_recovery_principal("recovery.e43.one")
            authorization = _governed_recovery_authorization(principal, "claim.e42.one", holder, "2026-08-02T12:05:00Z", "2026-08-02T12:10:00Z", "holder_timeout")
            authority.governed_recover_expired_claim(provenance, authorization, "2026-08-02T12:06:00Z")
    ...
    def test_durable_mutation_error_is_not_misreported_as_provenance_tamper(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
        ...
        self.assertEqual(result.code, DurableClaimResultCode.INVOCATION_MISMATCH)


def _e44_witness(challenge, *, status: CapabilityStatus = CapabilityStatus.SUPPORTED) -> CapabilityWitness:
...
class E44DurableChallengeTests(unittest.TestCase):
    ...
    def _finalized_claim(self, root: Path, holder: ClaimHolder | None = None, state: DurableClaimState = DurableClaimState.SUCCEEDED):
        ...
        attached = authority.attach_invocation(provenance, claimed.record.claim_id, holder, "invoke.e44.one")
        terminal_at = "2026-08-02T12:05:00Z"
        final = authority.finalize(provenance, claimed.record.claim_id, holder, state, "e44_terminal", terminal_at, invocation_id="invoke.e44.one")
        return final.record
    ...
    def test_durable_challenge_grants_only_fresh_supported_witness(self) -> None:
        ...
        self.assertEqual(evaluate_challenge_capability(CapabilityTarget.CODEX_APP, consumed.decision).status, CapabilityStatus.SUPPORTED)
    ...
    def test_challenge_binding_and_freshness_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            wrong = _e44_witness(challenge)
            wrong = CapabilityWitness(**{**wrong.__dict__, "nonce": "nonce.e44.other"})
            stale = CapabilityWitness(**{**_e44_witness(challenge).__dict__, "observed_at": "2026-08-02T12:03:00Z"})
    ...
    def test_manual_owner_terminal_requires_matching_owner_and_target(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            ledger.issue(challenge)
            capability = ledger.consume(challenge, _e44_witness(challenge), "2026-08-02T12:05:01Z").decision
            decision = validate_owner_terminal_evidence(claim, capability, _manual_terminal_evidence(claim))
        ...
        self.assertEqual(decision.evidence_type, "ManualAppTerminalEvidence")
    ...
    def test_automation_and_cli_schemas_are_owner_exclusive(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            auto_capability = auto_ledger.consume(auto_challenge, _e44_witness(auto_challenge), "2026-08-02T12:05:01Z").decision
            auto = AutomationTerminalEvidence(auto_claim.claim_id, auto_claim.invocation_id, automation_holder, CapabilityTarget.CODEX_APP, "completed", auto_claim.terminal_at, "dispatch.e44", "run.e44", "callback.e44", "callback.identity.e44", HASH, HASH, HASH)
            self.assertEqual(validate_owner_terminal_evidence(auto_claim, auto_capability, auto).evidence_type, "AutomationTerminalEvidence")
            ...
            cli_capability = cli_ledger.consume(cli_challenge, _e44_witness(cli_challenge), "2026-08-02T12:05:01Z").decision
            bad_cli = CliTerminalEvidence(cli_claim.claim_id, cli_claim.invocation_id, cli_holder, CapabilityTarget.CODEX_CLI, "completed", cli_claim.terminal_at, "launcher.e44", 42, HASH, 1, "CLEAN", HASH, HASH)
            with self.assertRaises(ValidationError):
    ...
    def test_e44_terminal_decision_is_required_for_reconciliation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            authority, provenance, holder, claimed = _claimed(Path(directory))
            attached = authority.attach_invocation(provenance, claimed.record.claim_id, holder, "invocation.e44.one")
            ...
            started = TerminalExecutionReconciler.begin(attached.record, challenge, "invocation.e44.one", "2026-08-02T12:04:00Z")
            ...
            final = authority.finalize(provenance, claimed.record.claim_id, holder, DurableClaimState.SUCCEEDED, "done", "2026-08-02T12:05:00Z", invocation_id="invocation.e44.one")
            with self.assertRaises(AttestationError):
                TerminalExecutionReconciler.reconcile(observed, final.record, "2026-08-02T12:05:02Z")
