...
E32_TASK = "CODEX-PEOS-0010-E31-FINAL-EXECUTABLE-HEAD-ARCHIVE-AND-SINGLE-RECEIPT-TOPOLOGY-CLOSURE-0024-E32"
E32_SIGNAL = "CODEX_E32_PEOS_0010_FINAL_EXECUTABLE_HEAD_ARCHIVE_AND_SINGLE_RECEIPT_TOPOLOGY_READY_FOR_GPT_REVIEW"
E32_BOUNDARY = "PUBLIC_SAFE / CANDIDATE_ONLY / DISABLED / research_only / NO_TRADE"
E32_ARCHIVE_SCHEMA = "VNAK_E32_ARCHIVE_PROVENANCE_v1"
E32_TOPOLOGY_SCHEMA = "VNAK_E32_TOPOLOGY_EVIDENCE_v1"
E32_RECEIPT_ALLOWLIST = (
    "coordination/PROGRAMS/SECOND-BRAIN-A-SHARE-ENTERPRISE-SYSTEM-0001/VENDOR-NEUTRAL-AGENT-OPERATING-KERNEL/AI_HANDOFF.yaml",
    "coordination/PROGRAMS/SECOND-BRAIN-A-SHARE-ENTERPRISE-SYSTEM-0001/VENDOR-NEUTRAL-AGENT-OPERATING-KERNEL/E32-ARCHIVE-PROVENANCE-MATRIX.yaml",
    "coordination/PROGRAMS/SECOND-BRAIN-A-SHARE-ENTERPRISE-SYSTEM-0001/VENDOR-NEUTRAL-AGENT-OPERATING-KERNEL/E32-COMPLETION-EVIDENCE.json",
    "coordination/PROGRAMS/SECOND-BRAIN-A-SHARE-ENTERPRISE-SYSTEM-0001/VENDOR-NEUTRAL-AGENT-OPERATING-KERNEL/E32-TEST-RUN-RECEIPT.md",
    "coordination/PROGRAMS/SECOND-BRAIN-A-SHARE-ENTERPRISE-SYSTEM-0001/VENDOR-NEUTRAL-AGENT-OPERATING-KERNEL/E32-TOPOLOGY-EVIDENCE.json",
...
def validate_e31_archive_manifest(
    path: Path,
    *,
    expected_task: str = E31_TASK,
    expected_schema: str = "VNAK_E31_ARCHIVE_PROVENANCE_v1",
) -> dict[str, Any]:
    manifest = _load_yaml(path)
    _require(manifest, "schema_version", "task_id", "status", "required_root_count", "tested_identity", "declared_artifact_surface", "roots")
    if manifest["schema_version"] != expected_schema or manifest["task_id"] != expected_task:
...
def validate_e32_archive_manifest(
    path: Path,
    *,
    ...
    """Validate the E32 runtime template or its committed final archive evidence."""
    manifest = _load_yaml(path)
    _require(manifest, "schema_version", "task_id", "status", "required_root_count", "tested_identity", "declared_artifact_surface")
    if manifest["schema_version"] != E32_ARCHIVE_SCHEMA or manifest["task_id"] != E32_TASK:
        raise ValueError("E32_ARCHIVE_IDENTITY_INVALID")
    tested = manifest["tested_identity"]
    if not isinstance(tested, dict):
        raise ValueError("E32_TESTED_IDENTITY_OBJECT_REQUIRED")
    if manifest["status"] == "RUNTIME_EXPECTED":
        if require_final:
            raise ValueError("E32_FINAL_ARCHIVE_REQUIRED")
        _require(tested, "binding", "commit", "tree")
        if tested["binding"] != "EXACT_CI_CONTEXT" or tested["commit"] != "CURRENT_CI_HEAD" or tested["tree"] != "CURRENT_CI_TREE":
            raise ValueError("E32_RUNTIME_ARCHIVE_TEMPLATE_INVALID")
        surface = manifest["declared_artifact_surface"]
        if not isinstance(surface, dict) or not isinstance(surface.get("paths"), list) or surface.get("path_count") != len(surface["paths"]):
            raise ValueError("E32_RUNTIME_ARCHIVE_SURFACE_INVALID")
        return {"status": "RUNTIME_EXPECTED", "root_count": 0, "artifact_count": 0}
    if manifest["status"] != "FINAL":
        raise ValueError("E32_ARCHIVE_STATUS_INVALID")
    summary = validate_e31_archive_manifest(
        path,
        expected_task=E32_TASK,
        expected_schema=E32_ARCHIVE_SCHEMA,
    )
    if tested_commit and summary["tested_commit"] != tested_commit:
        raise ValueError("E32_ARCHIVE_TESTED_COMMIT_MISMATCH")
    if tested_tree and manifest["tested_identity"].get("tested_tree") != tested_tree:
        raise ValueError("E32_ARCHIVE_TESTED_TREE_MISMATCH")
    return summary
...
def _git_parent(repository_root: Path, commit: str) -> str:
    try:
        raw = subprocess.check_output(
            ["git", "-C", str(repository_root), "cat-file", "-p", commit],
            text=True,
            stderr=subprocess.DEVNULL,
        )
    except (OSError, subprocess.CalledProcessError) as exc:
        raise ValueError("E32_GIT_OBJECT_UNAVAILABLE:" + commit) from exc
    parents = [line.split(" ", 1)[1] for line in raw.splitlines() if line.startswith("parent ")]
    if len(parents) != 1:
        raise ValueError("E32_RECEIPT_PARENT_COUNT_INVALID")
    return parents[0]


def _git_changed_paths(repository_root: Path, tested_commit: str, receipt_commit: str) -> tuple[str, ...]:
    try:
        raw = subprocess.check_output(
            ["git", "-C", str(repository_root), "diff", "--name-only", f"{tested_commit}..{receipt_commit}"],
            text=True,
            stderr=subprocess.DEVNULL,
        )
    except (OSError, subprocess.CalledProcessError) as exc:
        raise ValueError("E32_GIT_DIFF_UNAVAILABLE") from exc
    return tuple(sorted(line.strip().replace("\\", "/") for line in raw.splitlines() if line.strip()))


def validate_e32_topology(
    root: Path,
    *,
    current_commit: str,
    current_tree: str,
    ...
    """Enforce E32's one-tested-head plus one-evidence-receipt topology from Git objects."""
    topology = _load_json(root / "E32-TOPOLOGY-EVIDENCE.json")
    _require(topology, "schema", "task_id", "route_epoch", "status", "boundary", "tested_identity", "receipt_contract", "evidence_only_allowlist")
    if topology["schema"] != E32_TOPOLOGY_SCHEMA or topology["task_id"] != E32_TASK or topology["route_epoch"] != 33:
        raise ValueError("E32_TOPOLOGY_IDENTITY_INVALID")
    if topology["boundary"] != E32_BOUNDARY or topology["status"] not in {"TESTED_HEAD", "RECEIPT_HEAD"}:
        raise ValueError("E32_TOPOLOGY_STATUS_OR_BOUNDARY_INVALID")
    _require_sha(current_commit, "commit")
    _require_sha(current_tree, "tree")
    _require_sha(tested_commit, "commit")
    _require_sha(tested_tree, "tree")
    allowlist = topology["evidence_only_allowlist"]
    if tuple(sorted(allowlist)) != E32_RECEIPT_ALLOWLIST:
        raise ValueError("E32_RECEIPT_ALLOWLIST_INVALID")
    if current_commit == tested_commit:
        identity = topology["tested_identity"]
        receipt = topology["receipt_contract"]
        if identity != {"binding": "EXACT_CI_CONTEXT", "commit": "CURRENT_CI_HEAD", "tree": "CURRENT_CI_TREE"}:
            raise ValueError("E32_TESTED_HEAD_IDENTITY_INVALID")
        if receipt.get("parent_commit") != "CURRENT_TESTED_HEAD" or receipt.get("binding") != "NOT_YET_CREATED":
            raise ValueError("E32_TESTED_HEAD_RECEIPT_STATE_INVALID")
        if topology["status"] != "TESTED_HEAD":
            raise ValueError("E32_TESTED_HEAD_STATUS_INVALID")
        return {"phase": "TESTED_HEAD", "tested_commit": tested_commit, "receipt_parent": None, "changed_paths": ()}

    if topology["status"] != "RECEIPT_HEAD":
        raise ValueError("E32_RECEIPT_HEAD_STATUS_INVALID")
    identity = topology["tested_identity"]
    receipt = topology["receipt_contract"]
    _require(identity, "commit", "tree", "binding")
    _require(receipt, "parent_commit", "binding", "head_binding")
    if identity["binding"] != "EXACT_TESTED_HEAD" or identity["commit"] != tested_commit or identity["tree"] != tested_tree:
        raise ValueError("E32_TESTED_HEAD_IDENTITY_MISMATCH")
    if receipt["parent_commit"] != tested_commit or receipt["binding"] != "EXACT_TESTED_PARENT" or receipt["head_binding"] != "CURRENT_PR_HEAD":
        raise ValueError("E32_RECEIPT_PARENT_CONTRACT_INVALID")
    if _git_parent(root.parents[3], current_commit) != tested_commit:
        raise ValueError("E32_RECEIPT_PARENT_NOT_TESTED_HEAD")
    changed_paths = _git_changed_paths(root.parents[3], tested_commit, current_commit)
    if changed_paths != E32_RECEIPT_ALLOWLIST:
        raise ValueError("E32_RECEIPT_ALLOWLIST_VIOLATION")
    for path in changed_paths:
        if path.endswith(".py") or "/tests/" in f"/{path}" or path.startswith(".github/workflows/") or path.endswith(".schema.json") or path.endswith("requirements-ci.txt"):
            raise ValueError("E32_EXECUTABLE_CHANGE_AFTER_TESTED_HEAD")
    return {"phase": "RECEIPT_HEAD", "tested_commit": tested_commit, "receipt_parent": tested_commit, "changed_paths": changed_paths}
