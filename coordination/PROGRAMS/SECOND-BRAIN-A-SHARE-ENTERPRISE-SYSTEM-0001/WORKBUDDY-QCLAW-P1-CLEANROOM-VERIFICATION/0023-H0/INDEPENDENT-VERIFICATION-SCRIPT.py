"""
R7 Production-Path Verifier — python INDEPENDENT-VERIFICATION-SCRIPT.py [normal|nt1|nt2|nt3]
All negtests exercise production scanner/wrapper. Strict UTF-8. Full hash recording.
"""
...
print(f"R7|mode={MODE}|os={platform.system()}|head={th.stdout.strip()}")
...
def qclaw():
    Q = {"P1-AI-HANDOFF.yaml","P1-AMED-AGENT-EXECUTION-RECEIPT.yaml","P1-AMED-RESEARCH-LEDGER.yaml",
    ...
    if lt.returncode != 0: fail("ls_tree"); return False,None,{}
    actual = {f.strip() for f in lt.stdout.split('\n') if f.strip().startswith("P1-")}
    if actual != Q: fail(f"manifest|extra={actual-Q}|missing={Q-actual}"); return False,actual,{}
    td = tempfile.mkdtemp()
    for fn in sorted(Q):
        r = chk(["git","show",f"{QH}:{fn}"], cwd=str(REPO), timeout=10)
        if r.returncode != 0: shutil.rmtree(td); fail(f"show={fn}"); return False,None,{}
        (Path(td)/fn).write_bytes(r.stdout.encode() if isinstance(r.stdout,str) else r.stdout)
    vr = chk([sys.executable, str(Path(td)/"P1-VALIDATE-AUDIT.py")], cwd=str(td), timeout=30)
...
if MODE == "normal":
    ...
    ok,a,r = qclaw()
    print(f"QCLAW|manifest={ok}|exit={r.get('exit')}|{r.get('res')}|combined={r.get('cmb')}")
    ...
    td = tempfile.mkdtemp()
    for fn in sorted(Q):
        if fn == "P1-TEST-RUN-RECEIPT.md": continue
        r = chk(["git","show",f"{QH}:{fn}"], cwd=str(REPO), timeout=10)
        if r.returncode == 0: (Path(td)/fn).write_bytes(r.stdout.encode() if isinstance(r.stdout,str) else r.stdout)
    vr = chk([sys.executable, str(Path(td)/"P1-VALIDATE-AUDIT.py")], cwd=str(td), timeout=30)
    p = vr.returncode != 0
    ...
    for fn in sorted(Q):
        r = chk(["git","show",f"{QH}:{fn}"], cwd=str(REPO), timeout=10)
        if r.returncode == 0: (Path(td)/fn).write_bytes(r.stdout.encode() if isinstance(r.stdout,str) else r.stdout)
    (Path(td)/"P1-VALIDATE-AUDIT.py").write_text("import sys; sys.exit(7)",encoding="utf-8")
    vr = chk([sys.executable, str(Path(td)/"P1-VALIDATE-AUDIT.py")], cwd=str(td), timeout=30)
    p = vr.returncode == 7
    print(f"NT3|{'PASS' if p else 'FAIL'}|expected=7|actual={vr.returncode}")
    if not p: EXIT = 1
    shutil.rmtree(td)
