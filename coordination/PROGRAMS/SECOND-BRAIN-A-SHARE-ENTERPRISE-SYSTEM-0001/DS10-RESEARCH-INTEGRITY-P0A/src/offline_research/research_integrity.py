...
class ExperimentFamilySnapshot:
    ...
    def __post_init__(self):
        for name in ("experiment_family_ref", "benchmark_ref", "metric_id", "horizon_id", "search_space_ref",
                     "selection_rule_ref", "selected_trial_id", "lockbox_id"):
...
class LockboxAccessReceipt:
    ...
    def row(self) -> dict[str, Any]:
        return {"schema": LOCKBOX_SCHEMA, "access_id": self.access_id, "lockbox_id": self.lockbox_id,
                ...
                "selection_consumed_after": self.selection_consumed_after, "subsequent_action": self.subsequent_action}
...
def _selection(s: ExperimentFamilySnapshot):
    ...
    if _time(s.family_frozen_at, "family.family_frozen_at") > _time(s.selected_at, "family.selected_at"):
        f.append(_finding("FAMILY_FROZEN_AFTER_SELECTION", "BLOCKING", "family.family_frozen_at",
                          "family freeze post-dates winner"))
    if s.selected_trial_id not in s.expected_trial_digests:
        f.append(_finding("SELECTED_TRIAL_NOT_IN_REGISTERED_FAMILY", "BLOCKING", "family.selected_trial_id",
                          "winner absent from manifest"))
    return f
...
def _lockbox(s: ExperimentFamilySnapshot, receipts: Sequence[LockboxAccessReceipt]):
    ...
    bad = len(rows) != 1
    if bad:
        f.append(_finding("LOCKBOX_OPENED_MORE_THAN_ONCE", "BLOCKING", "lockbox_receipts",
                          "final holdout may be revealed once"))
    ...
    return (LockboxStatus.CONTAMINATED_REUSED_FOR_SELECTION if bad else LockboxStatus.OPENED_ONCE_FINAL_EVAL), f, out
...
def _methods(required: Sequence[str], results: Sequence[MethodResult]):
    ...
    for mid in required:
        ...
        elif r.status in {MethodStatus.FAIL, MethodStatus.NOT_RUN, MethodStatus.INSUFFICIENT_DATA,
                           MethodStatus.NUMERICAL_FAILURE}:
            blocked = True
            f.append(_finding("REQUIRED_METHOD_NOT_CLEAR", "BLOCKING", f"methods.{mid}.status",
...
def audit_research_integrity(snapshot: ExperimentFamilySnapshot, *, expected_w4_snapshot_digest: str | None,
    ...
    """P0A content-integrity audit. Caller-supplied digests never mint W4 authority."""
    ...
    findings += fs
    fs, pit_fail, pit_unknown = _pit(pit_evidence)
    findings += fs
    ...
    out = {
        ...
        "lockbox_status": lock_state.value, "lockbox_access_receipts": lock_rows, "pit_evidence": pit_evidence.row(),
