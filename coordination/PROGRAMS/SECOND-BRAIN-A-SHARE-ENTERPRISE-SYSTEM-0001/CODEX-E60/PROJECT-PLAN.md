...
The runtime validates schema, exact-head topology, payload digest, source manifest digest, and non-contradictory lifecycle state. It will reject a mismatched attestation, an attestation for another receipt, truncated SHAs, duplicate conflicting status, and a self-issued runtime attestation. Runtime identity canonicalizes Python source line endings before hashing, so the same Git source is not invalidated by a Windows CRLF checkout. Test success uses a fixed, pre-signed `SYNTHETIC_FIXTURE_ACCEPTED` record only; no signing private key is present in the runtime or test tree, and this fixture can neither represent nor authorize a real external review. A real `ACCEPTED_EXTERNAL` record requires a signed `GITHUB_PR_COMMENT:<numeric-id>` reference, which must be independently checked outside the runtime against the review record. The final external anchor is authored outside the tested code path after Provider results exist. No tested or receipt commit may state that such an anchor has already happened when it has not.

### 4.3 Whole-task resource lease

All local E60 test, mutation, child, and grandchild creation will pass through one outer `TaskResourceLease`. Its default is `FOREGROUND_PRIORITY`: no more than two E60-owned Python processes for a local lifecycle canary (one root plus one grandchild), one CPU-bound worker, below-normal process priority, serial inner execution, and a shared four-owned-process project hard cap. `IDLE_BATCH` is a conditional admission profile, not a default or a right to fan out: it requires three explicit clean low-load samples with no foreground contention and may allow only policy-level capacity up to three per agent; E60 lifecycle canaries remain capped at two in either profile. Sustained CPU pressure, memory pressure, user-reported stutter, or unexpected growth stops new local spawning and demotes immediately. Heavy matrices and high-concurrency validation are remote Provider work. The outer mutation runner is itself leased; inner tests do not create independent untracked leases.
