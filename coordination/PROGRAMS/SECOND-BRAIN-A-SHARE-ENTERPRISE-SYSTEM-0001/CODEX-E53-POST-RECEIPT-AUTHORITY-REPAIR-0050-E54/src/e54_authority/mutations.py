...
MUTATION_SPECS: tuple[MutationSpec, ...] = (
    ...
    MutationSpec("MUT-RECEIPT-SHA", "topology.py", "for field in (\"base_sha\", \"plan_sha\", \"tested_sha\"):", "for field in ():", "placeholder SHA shapes are accepted"),
    MutationSpec("MUT-RECEIPT-EXTERNAL-BINDING", "topology.py", "if binding.get(\"receipt_parent_sha\") != receipt[\"tested_sha\"]:", "if False:", "receipt binding can point to a parent other than the tested head"),
    ...
    MutationSpec("MUT-PROVIDER-MISSING-IDS", "provider.py", "if not isinstance(ids, list) or sorted(ids) != sorted(expected_mutation_ids) or len(ids) != len(set(ids)):", "if False:", "missing Provider mutation identifiers are accepted"),
    MutationSpec("MUT-PROVIDER-ARTIFACT", "provider.py", "if environment.get(\"canonical_artifact_sha256\") != expected_canonical_artifact_sha256:", "if False:", "changed Provider canonical artifact is accepted"),
)
