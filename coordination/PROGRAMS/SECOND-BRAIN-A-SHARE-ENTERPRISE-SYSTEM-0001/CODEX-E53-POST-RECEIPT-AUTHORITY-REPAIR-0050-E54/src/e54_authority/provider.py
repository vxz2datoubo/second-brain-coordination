...
def validate_environment_evidence(environment: Mapping[str, object], *, expected_head: str, expected_test_count: int, expected_mutation_ids: Sequence[str], expected_canonical_artifact_sha256: str | None = None) -> None:
    ...
    if expected_canonical_artifact_sha256 is not None:
        if not SHA256.fullmatch(expected_canonical_artifact_sha256):
            raise AuthorityError("expected canonical artifact digest is invalid")
        if environment.get("canonical_artifact_sha256") != expected_canonical_artifact_sha256:
            raise AuthorityError("Provider canonical artifact digest differs from the independently compared artifact")
...
def validate_matrix(environments: Sequence[Mapping[str, object]], *, expected_head: str, expected_test_count: int, expected_mutation_ids: Sequence[str], expected_canonical_artifact_sha256: str | None = None, expected_python_versions: Sequence[str] = ("3.11", "3.13"), expected_seeds: Sequence[str] = ("0", "1", "777")) -> None:
    ...
    for environment in environments:
        validate_environment_evidence(environment, expected_head=expected_head, expected_test_count=expected_test_count, expected_mutation_ids=expected_mutation_ids, expected_canonical_artifact_sha256=expected_canonical_artifact_sha256)
