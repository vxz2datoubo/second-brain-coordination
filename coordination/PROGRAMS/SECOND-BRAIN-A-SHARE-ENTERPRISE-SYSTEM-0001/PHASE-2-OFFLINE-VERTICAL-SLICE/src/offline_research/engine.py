...
def _session_from_time(event_time: str) -> str | None:
    ...
    # Synthetic fixtures use UTC timestamps as a simple clock token; preserve legacy 15:00 close bars.
    if hhmm == (15, 0):
...
class SimulationConfig:
    ...
    calendar_source_ref: str = "synthetic-fixture-calendar"
...
class ContractRuntime:
    ...
    def envelope(self, bar: Bar, run_id: str, trace_id: str) -> dict[str, Any]:
        return {
            "schema_version": "1.0.0", "object_id": bar.event_id, "record_type": "PriceBar",
            "run_id": run_id, "trace_id": trace_id, "no_trade_gate": True, "authority_write": False,
...
class SchemaRegistry:
    def __init__(self) -> None:
        self.known = {"FoundationSharedEnvelope": "1.0.0", "TemporalSemantics": "1.0.0", "OfflineReplay": "1.0.0", "C2_A_SHARE_RULE_SNAPSHOT": "1.0.0"}
...
def load_fixture(path: Path, requested_as_of: str) -> tuple[list[Bar], list[GateRecord], dict[str, Any]]:
    ...
    manifest = {"dataset_id": path.stem, "dataset_hash": hashlib.sha256(text.encode()).hexdigest(), "record_count": len(bars), "quarantine_count": len(quarantine), "license": "CC0-1.0 synthetic fixture", "synthetic": True, "formats_supported": ["csv", "json", "jsonl"]}
...
class DeterministicReplay:
    ...
    def run(self, bars: Iterable[Bar], checkpoint_path: Path | None = None, resume: bool = False) -> ReplayResult:
        seen, near_seen, quarantine, accepted = set(), {}, [], []
        for bar in bars:
            try:
                ...
                if bar.event_id in seen or fingerprint in seen:
                    quarantine.append(GateRecord(bar.event_id, "DUPLICATE", "idempotency_key_seen")); continue
                near_key = (bar.symbol, bar.event_time)
                if near_key in near_seen and abs(near_seen[near_key].close - bar.close) <= 0.001:
                    quarantine.append(GateRecord(bar.event_id, "NEAR_DUPLICATE", "same_symbol_time_near_price")); continue
                seen.update({bar.event_id, fingerprint}); near_seen[near_key] = bar; accepted.append(bar)
        ...
        for index, bar in enumerate(ordered):
            ...
            if checkpoint_path:
                ...
                checkpoint_path.write_text(canonical({"next_index": index + 1, "input_hash": input_hash, "last_event_id": bar.event_id, "run_id": self.run_id}))
        return ReplayResult(ordered, ledger, quarantine, {"next_index": len(ordered), "input_hash": input_hash, "run_id": self.run_id})
...
def candidate_signals(events: list[Bar]) -> list[dict[str, Any]]:
    ...
    for bar in events:
        if len(history) >= 2:
            unknown = sorted({f for item in (history[-2], history[-1], bar) for f in ("volume", "suspended", "is_st") if getattr(item, f) is None})
            if unknown:
                result.append({"signal_id": f"signal:{bar.event_id}", "symbol": bar.symbol, "event_id": bar.event_id, "available_at": bar.available_at, "action": "ABSTAIN", "confidence": 0.0, "features": {"momentum_2": None, "volume_ratio_2": None, "breakout_2": None}, "status": "candidate", "reason": "REQUIRED_MARKET_SEMANTICS_UNKNOWN", "unknown_fields": unknown, "failure_conditions": ["required_market_semantics_unknown", "no_execution_adapter"]})
                history.append(bar); continue
        if len(history) >= 2 and bar.suspended is False:
            ...
            result.append({"signal_id": f"signal:{bar.event_id}", "symbol": bar.symbol, "event_id": bar.event_id, "available_at": bar.available_at, "action": action, "confidence": round(min(.9, .35 + abs(score)*.15), 2), "features": {"momentum_2": round(momentum, 6), "volume_ratio_2": round(volume_ratio, 6), "breakout_2": breakout}, "status": "candidate", "failure_conditions": ["synthetic_dataset", "historical_bar_only", "no_execution_adapter"]})
        history.append(bar)
    return result


def _calendar_for_events(events: list[Bar], config: SimulationConfig) -> TradingCalendar:
    days = config.trading_days or tuple(sorted({bar.trading_day.isoformat() for bar in events}))
    return TradingCalendar("R143-SYNTHETIC-CALENDAR", "A_SHARE", tuple(days), config.calendar_source_ref)


def _reference_for_bar(bar: Bar, snapshot_id: str) -> LimitReferencePrice | None:
    if bar.limit_reference_price is None:
        return None
    return LimitReferencePrice(bar.limit_reference_price, bar.limit_reference_price_kind or "OFFICIAL_PREVIOUS_CLOSE", bar.limit_reference_source or "", bar.trading_day.isoformat(), snapshot_id)


def _fillability_evidence_for_bar(bar: Bar) -> FillabilityEvidence | None:
    if not bar.fillability_evidence_kind:
        return None
    if bar.fillability_observed is None:
        return None
    return FillabilityEvidence(bar.fillability_evidence_id or f"fill:{bar.event_id}", bar.fillability_evidence_source or bar.source_id, bar.observed_at, bar.fillability_observed, bar.fillability_evidence_kind, bar.contra_liquidity_quantity)
...
def simulate_portfolio(events: list[Bar], signals: list[dict[str, Any]], config: SimulationConfig) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """Research ledger with W2 PIT rule, fillability, session/calendar and lot-level T+1 gates."""
    ordered_signals = sorted(signals, key=lambda item: (parse_time(item["available_at"]), item["signal_id"]))
    signal_index, eligible, cash, turnover = 0, {}, config.initial_cash, 0.0
    inventory = SettlementAwareInventory()
    ledger, decisions = [], []
    calendar = _calendar_for_events(events, config)
    lot_counter = 0
    for bar in events:
        while signal_index < len(ordered_signals) and parse_time(ordered_signals[signal_index]["available_at"]) <= parse_time(bar.event_time):
            c = ordered_signals[signal_index]; eligible[c["symbol"]] = c; signal_index += 1
        ...
        if bar.volume is None or bar.suspended is None or bar.is_st is None:
            ...
            gate = replay_gate(resolver=DEFAULT_RULE_RESOLVER, exchange=_exchange_code(bar.exchange), board=bar.board, security_status=bar.security_status, trading_day=bar.trading_day.isoformat(), calendar=calendar, session=bar.replay_session, listing_trading_day_number=bar.listing_trading_day_number)
            if not gate.allowed:
                reason = gate.reason
            else:
                snapshot_id = gate.rule_snapshot_id
                snapshot = DEFAULT_RULE_RESOLVER.resolve(_exchange_code(bar.exchange), bar.board, bar.security_status, bar.trading_day.isoformat(), bar.listing_trading_day_number)
                try:
                    validity = evaluate_price_validity(bar.close, snapshot, _reference_for_bar(bar, snapshot.rule_snapshot_id), bar.trading_day)
                ...
                if validity is not None and not validity.valid:
                    ...
                    fillability = evaluate_order_fillability(validity, _fillability_evidence_for_bar(bar), config.conservative_no_fill_at_limit)
                ...
                if reason is None and action == "BUY_CANDIDATE":
                    ...
                    if quantity <= 0 or quantity * fill + fee > cash: reason = "INSUFFICIENT_CASH"
                    elif turnover + quantity * fill / config.initial_cash > config.max_turnover: reason = "MAX_TURNOVER"
                    else:
                        cash -= quantity * fill + fee; turnover += quantity * fill / config.initial_cash
                        lot_counter += 1; inventory.acquire(bar.symbol, quantity, bar.trading_day, f"lot-{lot_counter}"); executed = True
                elif reason is None and action == "SELL_CANDIDATE":
                    total = inventory.total_quantity(bar.symbol)
                    sellable = inventory.sellable_quantity(bar.symbol, bar.trading_day, calendar) if total else 0
                    if total <= 0: reason = "NO_POSITION"
                    elif sellable <= 0: reason = "T_PLUS_ONE_LOCK"
                    else:
                        quantity = sellable
                        fill = bar.close * (1 - (config.fixed_slippage_bps + config.volume_impact_bps) / 10_000)
                        ...
                        if _exchange_code(bar.exchange) == "SSE": fee += quantity * fill * config.transfer_fee_bps_sh / 10_000
                        inventory.sell(bar.symbol, quantity, bar.trading_day, calendar)
                        cash += quantity * fill - fee; turnover += quantity * fill / config.initial_cash; executed = True
        decision = {"event_id": bar.event_id, "symbol": bar.symbol, "action": action, "research_only": True, "no_trade_gate": config.no_trade_gate, "executed_in_simulation": executed, "reason": reason, "quantity": quantity, "rule_snapshot_id": snapshot_id, "price_validity_state": price_state, "order_fillability_state": fill_state}
        decisions.append(decision)
        ledger.append({**decision, "cash_after": round(cash, 2), "position_after": inventory.total_quantity(bar.symbol), "sellable_after": inventory.sellable_quantity(bar.symbol, bar.trading_day, calendar) if calendar.is_trading_day(bar.trading_day) and inventory.total_quantity(bar.symbol) else 0, "inventory_lots": inventory.snapshot(bar.symbol), "turnover_after": round(turnover, 6), "rule_version": config.rule_version, "zero_market_impact_baseline_assumption": config.volume_impact_bps == 0.0})
...
def validate(events: list[Bar], portfolio_ledger: list[dict[str, Any]], config: SimulationConfig) -> dict[str, Any]:
    unknown = sorted({f for bar in events for f in ("volume", "suspended", "is_st") if getattr(bar, f) is None})
    if unknown:
        return {"validation_status": "ABSTAIN", "reason": "required_market_semantics_unknown", "unknown_fields": unknown, "executed_simulated_actions": 0, "cost_proxy": 0.0, "research_only": True}
    if len(events) < 6:
        return {"validation_status": "ABSTAIN", "reason": "insufficient_temporal_observations", "research_only": True}
    ordered = sorted(events, key=lambda item: parse_time(item.event_time)); n = len(ordered)
    ...
    executed = [e for e in portfolio_ledger if e["executed_in_simulation"]]; rejected = [e for e in portfolio_ledger if not e["executed_in_simulation"]]
    return {"validation_status": "EXPERIMENTAL_ONLY", "time_split": split, "walk_forward_windows": max(1, n-4), "random_shuffle": False, "executed_simulated_actions": len(executed), "abstentions_or_rejections": len(rejected), "cost_proxy": len(executed)*config.min_commission, "cost_before_result": "not_economic_evidence", "cost_after_result": "not_economic_evidence", "robustness": "not_proven_on_synthetic_fixture", "research_only": True, "zero_market_impact_baseline_assumption": config.volume_impact_bps == 0.0}


@dataclass
class KnowledgeAtom:
    atom_id: str; content: str; status: str; source_refs: list[str]; evidence_quality: str
    gpt_access: str = "FULL_SEMANTIC_ACCESS"; transport_visibility: str = "PUBLIC_SAFE"; relations: list[dict[str, str]] = field(default_factory=list)


class KnowledgeGateway:
    HARD_SECRET_MARKERS = ("api_key", "token=", "password", "private_key", "cookie")
    def __init__(self, atoms: list[KnowledgeAtom], revision: str = "synthetic-r1") -> None: self.atoms, self.revision = atoms, revision
    def query(self, text: str, budget: int, statuses: set[str] | None = None) -> dict[str, Any]:
        if any(m in text.lower() for m in self.HARD_SECRET_MARKERS): return {"query_id": digest(text)[:16], "abstention": "DENIED_HARD_SECRET", "atoms": [], "omitted_due_to_context_budget": [], "knowledge_revision": self.revision}
        allowed, tokens, chosen, omitted, used = statuses or {"candidate", "approved"}, set(text.lower().split()), [], [], 0
        for atom in sorted(self.atoms, key=lambda x: x.atom_id):
            if atom.status not in allowed or atom.gpt_access != "FULL_SEMANTIC_ACCESS" or not tokens.intersection(atom.content.lower().split()): continue
            cost = max(1, len(atom.content)//4)
            if used + cost > budget: omitted.append(atom.atom_id); continue
            chosen.append(asdict(atom)); used += cost
        return {"query_id": digest({"text": text, "budget": budget})[:16], "knowledge_revision": self.revision, "atoms": chosen, "relations": [r for a in chosen for r in a["relations"]], "conflicts": [], "unknowns": [], "source_lineage": [s for a in chosen for s in a["source_refs"]], "omitted_due_to_context_budget": omitted, "context_budget_report": {"budget": budget, "used": used}, "gpt_access": "FULL_SEMANTIC_ACCESS"}


def learning_packet(run_manifest: dict[str, Any], validation_report: dict[str, Any], evidence_hash: str) -> dict[str, Any]:
    packet = {"packet_id": "lp-" + digest({"run": run_manifest["run_id"], "evidence": evidence_hash})[:16], "packet_content_hash": "", "idempotency_key": "", "base_knowledge_revision": "synthetic-r1", "processor_version": "p2-offline-r143", "status": "candidate", "authority_write": False, "facts": ["offline synthetic replay completed"], "constraints": ["research_only", "NO_TRADE", "synthetic_not_market_evidence"], "validation": validation_report, "evidence_refs": [evidence_hash], "evidence_quality": "synthetic_fixture", "relations": [{"type": "evidence:SUPPORTS", "target": run_manifest["run_id"]}]}
    packet["packet_content_hash"] = digest({k:v for k,v in packet.items() if k not in {"packet_content_hash", "idempotency_key"}}); packet["idempotency_key"] = packet["packet_id"] + "-" + packet["packet_content_hash"][:12]; return packet


class OfflineResearchRunner:
    def __init__(self, fixture: Path, output_dir: Path, requested_as_of: str = "2026-01-31T23:59:59Z") -> None:
        self.fixture, self.output_dir, self.requested_as_of = fixture, output_dir, requested_as_of
        self.run_id = "run-" + digest({"fixture": fixture.name, "as_of": requested_as_of})[:16]; self.trace_id = "trace-" + self.run_id[-12:]
    def run(self, resume: bool = False) -> dict[str, Any]:
        ...
        signals = candidate_signals(replay.events); cfg = SimulationConfig(); portfolio, decisions = simulate_portfolio(replay.events, signals, cfg); report = validate(replay.events, portfolio, cfg)
        knowledge = KnowledgeGateway([KnowledgeAtom("atom-signal", "candidate momentum volume breakout signal from synthetic replay", "candidate", [dataset_manifest["dataset_hash"]], "synthetic_fixture"), KnowledgeAtom("atom-risk", "T+1 suspension price validity fillability and cost constraints remain research only", "candidate", ["C2_A_SHARE_RULE_SNAPSHOT"], "synthetic_fixture")])
        context = knowledge.query("momentum volume T+1", 200)
        run_manifest = {"run_id": self.run_id, "trace_id": self.trace_id, "research_only": True, "no_trade_gate": True, "fixture": self.fixture.name, "dataset_hash": dataset_manifest["dataset_hash"], "event_hash": replay.core_hash, "code_version": "p2-offline-r143"}
        evidence = {"run_manifest": run_manifest, "dataset_manifest": dataset_manifest, "capability_decisions": [asdict(x) for x in ingest_quarantine + replay.quarantine], "replay_event_ledger": replay.event_ledger, "research_decision_ledger": decisions, "portfolio_ledger": portfolio, "validation_report": report, "context_bundle": context, "unknowns": ["Synthetic fixture is not a real-market evidence source", "No private/local knowledge gateway is implemented in public repository"]}
        evidence_hash = digest(evidence); packet = learning_packet(run_manifest, report, evidence_hash)
        artifacts = {"RunManifest.json": run_manifest, "DatasetManifest.json": dataset_manifest, "ConfigurationSnapshot.json": asdict(cfg), "CapabilityDecisionLog.json": evidence["capability_decisions"], "ReplayEventLedger.json": replay.event_ledger, "ResearchDecisionLedger.json": decisions, "PortfolioLedger.json": portfolio, "ValidationReport.json": report, "EvidenceLedger.json": evidence, "ContextBundle.json": context, "LearningPacket.json": packet}
        hashes = {}
