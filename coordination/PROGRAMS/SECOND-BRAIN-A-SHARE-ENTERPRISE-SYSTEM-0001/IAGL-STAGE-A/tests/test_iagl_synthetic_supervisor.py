...
class R141R2AndB13B15Regressions(unittest.TestCase):
    ...
    def test_b14_p0_user_disposition_requires_fresh_reconcile_preserves_trace_and_safe_work_recovers(self) -> None:
        ...
        binding = self.store.current_p0_approval(event.semantic_key); self.assertIsNotNone(binding)
        disposition = P0Disposition(event.semantic_key, "APPROVED_SEPARATE_GATED_ACTION", "user-decision:approve-separate-gate-1", binding.occurrence_key, binding.approval_epoch)
