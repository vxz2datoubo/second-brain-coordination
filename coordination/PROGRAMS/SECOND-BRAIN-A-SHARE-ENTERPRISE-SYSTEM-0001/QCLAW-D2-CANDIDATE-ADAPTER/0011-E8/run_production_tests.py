#!/usr/bin/env python3
'''
E21 Gate B R6 - PORTABLE Machine-Evidence Runner
Zero absolute-path hardcodes. Self-resolves: Python from sys.executable,
BASE from __file__, Q0 from BASE/q0_sources/, scripts from same directory.
Archive-independent: can run from any git-archive root.
>=42 adversarial subprocess mutation tests + 3-archive evidence.
'''
import hashlib, json, os, shutil, subprocess, sys, tempfile, time, yaml
from pathlib import Path

# ---- PORTABLE path resolution ----
PY = sys.executable  # Don't hardcode; use whatever Python runs THIS script
BASE = Path(__file__).resolve().parent  # Runner directory = 0011-E8
Q0_SRC = BASE / "q0_sources"  # Self-contained: Q0 shipped alongside
...
MF = [
    ...
    "PERSON-EVIDENCE-AUDIT.yaml", "GOLDEN-VECTORS.yaml"
]
QF = [
...
# ---- E21 Portable UTF-8: Patch Path read/write for archive independence ----
_ort = Path.read_text
_owt = Path.write_text
def _urt(self, *a, **kw):
    kw.setdefault("encoding", "utf-8")
    return _ort(self, *a, **kw)
def _uwt(self, data, *a, **kw):
    kw.setdefault("encoding", "utf-8")
    return _owt(self, data, *a, **kw)
Path.read_text = _urt
Path.write_text = _uwt

def sb(b):
    h = hashlib.sha256(); h.update(b if isinstance(b, bytes) else b.encode())
    return h.hexdigest()
...
def rc(cid, desc, act, cmd, ec, so, se, exp):
    ...
    RS.append({
        ...
        "command": cmd_str[:200], "exit_code": ec,
        "expected_fail": exp, "actual_result": "PASS" if ec else "FAIL" if not exp else "PASS",
        "stdout_sha256": sb(so or "")[:64], "stderr_sha256": sb(se or "")[:64],
        "passed": p, "adversarial": True,
        "runner_python": sys.version,
        "runner_executable": PY,
    })
    return p

def su(n):
    d = Path(tempfile.mkdtemp(prefix=f"e21_{n}_"))
    for f in QF:
        s = Q0_SRC / f
...
def T30():
    ...
    for i,l in enumerate(ls):
     ...
     if csr:
      for k in csr:
       if isinstance(csr[k],str) and len(csr[k])>10:csr[k]="TAMPERED_E21";break
...
def main():
    print("=" * 70)
    print(f"E21 Gate B R6 - {len(TESTS)} adversarial tests (PORTABLE runner)")
    ...
    rp = BASE / "e21_test_results.json"
    results_blob = {
        "test_run": {
            "runner_sha256": sf(__file__), "epoch": 21, "gate": "B R6",
    ...
    rl = [
        "# E21 Gate B R6 TEST-RUN-RECEIPT",
        "",
        "## Machine-Generated Direct-Derived Transcript",
        f"**Runner SHA256:** `{sf(__file__)}`",
        f"**Python:** `{sys.executable}`",
        f"**BASE:** `{BASE}`",
        f"**Generated:** {time.strftime('%Y-%m-%dT%H:%M:%S+08:00')}",
        "",
        "> This receipt is directly derived from actual machine test results.",
        "> No placeholders, no stale claims, no manual edits of results.",
        "",
        "## Results",
        "| Case ID | Description | Exit | Expected Fail | Passed |",
        "|---------|-------------|------|---------------|--------|",
    ]
    for r in RS:
        rl.append(f"| {r['case_id']} | {r['description'][:80]} | {r['exit_code']} | {r['expected_fail']} | {'PASS' if r['passed'] else 'FAIL'} |")
    rl.extend([
        "", "## Summary",
        f"- **Total:** {t}", f"- **Passed:** {p}", f"- **Failed:** {f_cnt}",
        "", f"## Results JSON SHA-256", f"`{sf(rp)}`",
        "", "## Completion",
        "QCLAW_E21_PR100_PORTABLE_RUNNER_ARCHIVE_PROVENANCE_AND_CUMULATIVE_HANDOFF_READY_FOR_GPT_REVIEW"
    ])
    (BASE / "E21-TEST-RUN-RECEIPT.md").write_text("\n".join(rl), encoding="utf-8")
    print(f"\n{'='*70}\nRESULTS: {p}/{t} passed\nJSON: {rp}\nReceipt: {BASE/'E21-TEST-RUN-RECEIPT.md'}\n{'='*70}")
    return 0 if f_cnt == 0 else 1
