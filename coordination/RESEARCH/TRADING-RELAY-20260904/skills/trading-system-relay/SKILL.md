...
- Reuse the repository's `PHASE-2-OFFLINE-VERTICAL-SLICE/run_demo.py` through `coordination/RESEARCH/TRADING-RELAY-20260904/scripts/prove_replay.py` in the fresh repository checkout. Read its `--help`, and pass the actual invoking agent through `--agent-id`. This installed skill is guidance; its references do not contain a second engine. Use only the synthetic fixture. Set `-X utf8` on Windows. Require a new output directory, source commit, source and input hashes, fixed as-of, runtime version, two actual subprocess runs and artifact equality.
