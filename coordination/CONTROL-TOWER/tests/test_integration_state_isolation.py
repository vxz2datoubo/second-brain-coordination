...
SCHEMA = json.loads((ROOT / "integration_state_isolation.schema.json").read_text(encoding="utf-8"))


def snapshot(rows: list[tuple[str, str, str | None]], *, python_version: str = "3.13.0") -> dict:
    import hashlib
    ...
    results = [
        {"test_id": test_id, "status": status, "failure_fingerprint": fingerprint}
    ...
    return {
        ...
        "test_id_digest": hashlib.sha256(stable(ids).encode()).hexdigest(),
        "state_digest": hashlib.sha256(stable(results).encode()).hexdigest(),
        "environment_errors": [],
        "results": results,
    }
...
class IntegrationStateIsolationTests(unittest.TestCase):
    def test_preserved_baseline_failure_is_visible_but_not_candidate_regression(self):
        ...
        after = snapshot([("t.pass", "PASS", None), ("t.base", "FAIL", fp), ("t.new", "PASS", None)])
        receipt = compare_snapshots(before, after)
        self.assertEqual(receipt["classification"], FINAL_PASS)
        self.assertEqual([x["test_id"] for x in receipt["baseline_failures_preserved"]], ["t.base"])
    ...
    def test_candidate_only_test_must_pass(self):
        ...
        after = snapshot([("old", "PASS", None), ("new", "ERROR", "c" * 64)])
        receipt = compare_snapshots(before, after)
    ...
    def test_explicit_environment_error_is_fail_closed(self):
        ...
        after = snapshot([("a", "PASS", None)])
        receipt = compare_snapshots(before, after, environment_errors=["TRUSTED_REPOSITORY_WORKTREE_DIRTY"])
        self.assertEqual(receipt["classification"], "TEST_ENVIRONMENT_INVALID")
    ...
    def test_receipt_is_deterministic_for_same_inputs(self):
        before = snapshot([("a", "PASS", None), ("b", "FAIL", "a" * 64)])
        after = snapshot([("a", "PASS", None), ("b", "FAIL", "a" * 64)])
        meta = {
        ...
        self.assertEqual(compare_snapshots(before, after, metadata=meta), compare_snapshots(before, after, metadata=meta))

    def test_authority_flags_are_all_false(self):
        receipt = compare_snapshots(snapshot([("a", "PASS", None)]), snapshot([("a", "PASS", None)]))
    ...
    def test_main_or_candidate_drift_can_be_injected_as_environment_invalid(self):
        ...
        receipt = compare_snapshots(before, after, environment_errors=["CURRENT_MAIN_DRIFT_DURING_PROOF", "CANDIDATE_HEAD_DRIFT_DURING_PROOF"])
        self.assertEqual(receipt["classification"], "TEST_ENVIRONMENT_INVALID")
