"""Public-safe fixed runner template for CLTM-0021 epoch 92 private-root post-permission verification.

No private path, candidate content, or secret is embedded here. The executable copy must be materialized
under OS temp from the exact remote commit and hash-verified before private-root access.
"""
...
SCHEMA_VERSION = "104.0"
PLAIN_MARKER = b"CLTM_R92_WRITE_PROBE"
R90_LEAF = "cltm-private-canary-r90.sqlite3"
PLAIN_LEAF = ".cltm-private-write-probe-r92.tmp"
SQLITE_LEAF = "cltm-private-memorystore-probe-r92.sqlite3"
...
def main() -> None:
    ...
    if plain.exists():
        receipt["status"] = "PRIVATE_STORE_VERIFY_PROBE_NOT_FRESH"
    ...
    try:
        ...
        receipt["status"] = "PRIVATE_ROOT_PLAIN_WRITE_VERIFY_FAILED"
    ...
    try:
        ...
        receipt["status"] = "PRIVATE_ROOT_PLAIN_WRITE_VERIFY_FAILED" if cleanup_ok else "PRIVATE_STORE_VERIFY_CLEANUP_FAILED"
    ...
    try:
        ...
        receipt["status"] = "PRIVATE_STORE_VERIFY_CLEANUP_FAILED"
    ...
    try:
        ...
        receipt["status"] = "PRIVATE_ROOT_MEMORYSTORE_SQLITE_VERIFY_FAILED"
    ...
    try:
        ...
        receipt["sqlite_memorystore_probe"] = "FAILED"
        receipt["cleanup_status"] = "PASS" if cleanup_ok else "CLEANUP_FAILED"
        receipt["status"] = "PRIVATE_ROOT_MEMORYSTORE_SQLITE_VERIFY_FAILED" if cleanup_ok else "PRIVATE_STORE_VERIFY_CLEANUP_FAILED"
    ...
    receipt["cleanup_status"] = "PASS" if cleanup_ok else "CLEANUP_FAILED"
    receipt["status"] = "PRIVATE_ROOT_PERMISSION_AND_MEMORYSTORE_VERIFY_PASS" if cleanup_ok else "PRIVATE_STORE_VERIFY_CLEANUP_FAILED"
    if not cleanup_ok:
