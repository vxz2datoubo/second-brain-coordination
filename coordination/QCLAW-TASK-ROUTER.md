...
5. 仅执行 `status: READY`、依赖已满足、影响预测存在且AMED字段完整的活动任务。
6. 必须读取活动Issue正文、全部评论、AMED协议、机器策略、任务影响预测和任务简报。
...
11. 执行必须同时完成主交付、主动发现和系统演进提案。重点寻找来源冲突、错误假设、知识缺口、可泛化技能、重复术语和反例。
12. 计划外改良按AMED A/B/C/D处理：QCLAW通常只允许A类候选包内部改良和C类提案；新Skill、权威升级、跨模块接口和运行时实现禁止自行执行。
13. 研究按L1/L2/L3触发。L2必须记录一手来源、反证、适用条件、来源冲突、可信度、许可和A股适配；L3形成独立研究任务候选。
14. 完成后必须回传结构化握手、LearningPacket、AMED执行回执、研究账本、计划外改良账本、系统发现报告、隐私审查、UNKNOWN、AI_HANDOFF和结果校准。
15. 不得自行合并PR、升级权威状态、改变活动任务或扩大到本地私人知识。
16. 无法确认远端最新索引、AMED字段、身份、隐私等级或任务边界时必须停止并报告，不得猜测。
17. 主动发现不授予访问秘密、绕过许可、真实交易或接管其他Agent任务的权限。
