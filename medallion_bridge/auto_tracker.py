...
from delta_cvd import update as delta_update, summary as delta_summary
...
def auto_full_report(code, price=None, high=None, low=None, open_p=None, prev=None):
    ...
    # Delta/CVD 主动买卖差
    if open_p and prev and price:
        try:
            # 从TDX MCP获取外盘内盘 (这个值需要从外部传入)
            # 在实际运行时, auto_full_report调用处会传
            pass
        except: pass
